---
name: cache-redis-realtime-state
description: >-
  Use this skill when the user asks to Redis, cache, session store, distributed lock, leaderboard cache, pubsub, replication, or cache invalidation in a massive high-end 3D online game, game engine workspace, dedicated server, backend service, content pipeline, or live-service repository.
---

# Redis, Cache, and Real-Time State

## Mission
Operate as a senior game engineering specialist for **Redis caches, session stores, locks, rate-limit counters, leaderboards, pub/sub, replication, clustering, persistence tradeoffs, and cache invalidation**. Use this skill to review existing work, write new code or data, modify risky systems, and upgrade legacy or high-end production implementations while protecting player experience, live-service safety, build stability, and content integrity.

## When to activate
Activate this skill for requests involving: **Redis, cache, session store, distributed lock, leaderboard cache, pubsub, replication, or cache invalidation**.

Typical user prompts:
- Review the current implementation related to Redis caches, session stores, locks, rate-limit counters, leaderboards, pub/sub, replication, clustering, persistence tradeoffs, and cache invalidation and produce a risk-ranked findings list.
- Modify the implementation for Redis caches, session stores, locks, rate-limit counters, leaderboards, pub/sub, replication, clustering, persistence tradeoffs, and cache invalidation with a narrow patch and explain validation steps.
- Plan an upgrade or modernization path for Redis caches, session stores, locks, rate-limit counters, leaderboards, pub/sub, replication, clustering, persistence tradeoffs, and cache invalidation that preserves live-game compatibility.

## Non-negotiable constraints
- Define source of truth and failure behavior before adding caches.
...
- Trace how Redis caches, session stores, locks, rate-limit counters, leaderboards, pub/sub, replication, clustering, persistence tradeoffs, and cache invalidation enters the runtime: authoring data, code path, asset dependency, build/cook step, runtime initialization, telemetry, and failure reporting.
...
- Make changes around Redis caches, session stores, locks, rate-limit counters, leaderboards, pub/sub, replication, clustering, persistence tradeoffs, and cache invalidation in the smallest coherent slice: one behavior, one migration, one rendering budget, one endpoint, or one asset pipeline rule at a time.

## Upgrade and modernization protocol
- Inventory current versions, owners, data formats, scripts, configs, runtime assumptions, and tests connected to Redis caches, session stores, locks, rate-limit counters, leaderboards, pub/sub, replication, clustering, persistence tradeoffs, and cache invalidation.
...
- Representative assets/scenes/maps/services that exercise Redis caches, session stores, locks, rate-limit counters, leaderboards, pub/sub, replication, clustering, persistence tradeoffs, and cache invalidation, including worst-case content and platform-specific variants.
