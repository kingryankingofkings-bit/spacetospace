---
name: visual-regression-screenshot-testing
description: >-
  Use this skill when the user asks to visual regression, screenshot test, golden image, compare images, render regression, or visual QA in a massive high-end 3D online game, game engine workspace, dedicated server, backend service, content pipeline, or live-service repository.
---

# Visual Regression and Screenshot Testing

## Mission
Operate as a senior game engineering specialist for **automated screenshot/video comparisons, golden images, camera bookmarks, content baselines, image tolerance, and visual QA for rendering or art changes**. Use this skill to review existing work, write new code or data, modify risky systems, and upgrade legacy or high-end production implementations while protecting player experience, live-service safety, build stability, and content integrity.

## When to activate
Activate this skill for requests involving: **visual regression, screenshot test, golden image, compare images, render regression, or visual QA**.

Typical user prompts:
- Review the current implementation related to automated screenshot/video comparisons, golden images, camera bookmarks, content baselines, image tolerance, and visual QA for rendering or art changes and produce a risk-ranked findings list.
- Modify the implementation for automated screenshot/video comparisons, golden images, camera bookmarks, content baselines, image tolerance, and visual QA for rendering or art changes with a narrow patch and explain validation steps.
- Plan an upgrade or modernization path for automated screenshot/video comparisons, golden images, camera bookmarks, content baselines, image tolerance, and visual QA for rendering or art changes that preserves live-game compatibility.

## Non-negotiable constraints
- Use deterministic cameras, fixed time-of-day, stable random seeds, and platform-specific tolerances.
...
- Trace how automated screenshot/video comparisons, golden images, camera bookmarks, content baselines, image tolerance, and visual QA for rendering or art changes enters the runtime: authoring data, code path, asset dependency, build/cook step, runtime initialization, telemetry, and failure reporting.
...
- Make changes around automated screenshot/video comparisons, golden images, camera bookmarks, content baselines, image tolerance, and visual QA for rendering or art changes in the smallest coherent slice: one behavior, one migration, one rendering budget, one endpoint, or one asset pipeline rule at a time.

## Upgrade and modernization protocol
- Inventory current versions, owners, data formats, scripts, configs, runtime assumptions, and tests connected to automated screenshot/video comparisons, golden images, camera bookmarks, content baselines, image tolerance, and visual QA for rendering or art changes.
...
- Representative assets/scenes/maps/services that exercise automated screenshot/video comparisons, golden images, camera bookmarks, content baselines, image tolerance, and visual QA for rendering or art changes, including worst-case content and platform-specific variants.
