---
name: vehicles-mounts-traversal
description: >-
  Use this skill when the user asks to vehicle, mount, traversal, flight, boat, driving, climbing, grapple, or movement gadget in a massive high-end 3D online game, game engine workspace, dedicated server, backend service, content pipeline, or live-service repository.
---

# Vehicles, Mounts, and Traversal

## Mission
Operate as a senior game engineering specialist for **vehicle controllers, mounts, flight, boats, traversal gadgets, collision, camera, animation sync, prediction, and server validation**. Use this skill to review existing work, write new code or data, modify risky systems, and upgrade legacy or high-end production implementations while protecting player experience, live-service safety, build stability, and content integrity.

## When to activate
Activate this skill for requests involving: **vehicle, mount, traversal, flight, boat, driving, climbing, grapple, or movement gadget**.

Typical user prompts:
- Review the current implementation related to vehicle controllers, mounts, flight, boats, traversal gadgets, collision, camera, animation sync, prediction, and server validation and produce a risk-ranked findings list.
- Modify the implementation for vehicle controllers, mounts, flight, boats, traversal gadgets, collision, camera, animation sync, prediction, and server validation with a narrow patch and explain validation steps.
- Plan an upgrade or modernization path for vehicle controllers, mounts, flight, boats, traversal gadgets, collision, camera, animation sync, prediction, and server validation that preserves live-game compatibility.

## Non-negotiable constraints
- Validate physics/networking/camera interactions under high speed and streaming.
...
- Trace how vehicle controllers, mounts, flight, boats, traversal gadgets, collision, camera, animation sync, prediction, and server validation enters the runtime: authoring data, code path, asset dependency, build/cook step, runtime initialization, telemetry, and failure reporting.
...
- Make changes around vehicle controllers, mounts, flight, boats, traversal gadgets, collision, camera, animation sync, prediction, and server validation in the smallest coherent slice: one behavior, one migration, one rendering budget, one endpoint, or one asset pipeline rule at a time.

## Upgrade and modernization protocol
- Inventory current versions, owners, data formats, scripts, configs, runtime assumptions, and tests connected to vehicle controllers, mounts, flight, boats, traversal gadgets, collision, camera, animation sync, prediction, and server validation.
...
- Representative assets/scenes/maps/services that exercise vehicle controllers, mounts, flight, boats, traversal gadgets, collision, camera, animation sync, prediction, and server validation, including worst-case content and platform-specific variants.
