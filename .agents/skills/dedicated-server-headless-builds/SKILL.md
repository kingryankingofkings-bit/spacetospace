---
name: dedicated-server-headless-builds
description: >-
  Use this skill when the user asks to dedicated server, headless, server build, container, game server, health check, or command line server in a massive high-end 3D online game, game engine workspace, dedicated server, backend service, content pipeline, or live-service repository.
---

# Dedicated Server and Headless Builds

## Mission
Operate as a senior game engineering specialist for **headless server builds, command-line startup, map rotation, containerization, health checks, logs, config, graceful shutdown, and compatibility with client builds**. Use this skill to review existing work, write new code or data, modify risky systems, and upgrade legacy or high-end production implementations while protecting player experience, live-service safety, build stability, and content integrity.

## When to activate
Activate this skill for requests involving: **dedicated server, headless, server build, container, game server, health check, or command line server**.

Typical user prompts:
- Review the current implementation related to headless server builds, command-line startup, map rotation, containerization, health checks, logs, config, graceful shutdown, and compatibility with client builds and produce a risk-ranked findings list.
- Modify the implementation for headless server builds, command-line startup, map rotation, containerization, health checks, logs, config, graceful shutdown, and compatibility with client builds with a narrow patch and explain validation steps.
- Plan an upgrade or modernization path for headless server builds, command-line startup, map rotation, containerization, health checks, logs, config, graceful shutdown, and compatibility with client builds that preserves live-game compatibility.

## Non-negotiable constraints
- Keep server-only code/content separated and executable in CI.
...
- Trace how headless server builds, command-line startup, map rotation, containerization, health checks, logs, config, graceful shutdown, and compatibility with client builds enters the runtime: authoring data, code path, asset dependency, build/cook step, runtime initialization, telemetry, and failure reporting.
...
- Make changes around headless server builds, command-line startup, map rotation, containerization, health checks, logs, config, graceful shutdown, and compatibility with client builds in the smallest coherent slice: one behavior, one migration, one rendering budget, one endpoint, or one asset pipeline rule at a time.

## Upgrade and modernization protocol
- Inventory current versions, owners, data formats, scripts, configs, runtime assumptions, and tests connected to headless server builds, command-line startup, map rotation, containerization, health checks, logs, config, graceful shutdown, and compatibility with client builds.
...
- Representative assets/scenes/maps/services that exercise headless server builds, command-line startup, map rotation, containerization, health checks, logs, config, graceful shutdown, and compatibility with client builds, including worst-case content and platform-specific variants.
