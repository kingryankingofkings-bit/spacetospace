---
name: persistence-database-schema-migrations
description: >-
  Use this skill when the user asks to database, migration, schema, SQL, NoSQL, index, transaction, backup, or player state in a massive high-end 3D online game, game engine workspace, dedicated server, backend service, content pipeline, or live-service repository.
---

# Persistence, Database, and Schema Migrations

## Mission
Operate as a senior game engineering specialist for **Postgres/MySQL/NoSQL schema design, migrations, indexes, transactions, consistency, backups, data retention, and player-state durability**. Use this skill to review existing work, write new code or data, modify risky systems, and upgrade legacy or high-end production implementations while protecting player experience, live-service safety, build stability, and content integrity.

## When to activate
Activate this skill for requests involving: **database, migration, schema, SQL, NoSQL, index, transaction, backup, or player state**.

Typical user prompts:
- Review the current implementation related to Postgres/MySQL/NoSQL schema design, migrations, indexes, transactions, consistency, backups, data retention, and player-state durability and produce a risk-ranked findings list.
- Modify the implementation for Postgres/MySQL/NoSQL schema design, migrations, indexes, transactions, consistency, backups, data retention, and player-state durability with a narrow patch and explain validation steps.
- Plan an upgrade or modernization path for Postgres/MySQL/NoSQL schema design, migrations, indexes, transactions, consistency, backups, data retention, and player-state durability that preserves live-game compatibility.

## Non-negotiable constraints
- Migrations must be backward-compatible, reversible or recoverable, and tested with realistic data volumes.
...
- Trace how Postgres/MySQL/NoSQL schema design, migrations, indexes, transactions, consistency, backups, data retention, and player-state durability enters the runtime: authoring data, code path, asset dependency, build/cook step, runtime initialization, telemetry, and failure reporting.
...
- Make changes around Postgres/MySQL/NoSQL schema design, migrations, indexes, transactions, consistency, backups, data retention, and player-state durability in the smallest coherent slice: one behavior, one migration, one rendering budget, one endpoint, or one asset pipeline rule at a time.

## Upgrade and modernization protocol
- Inventory current versions, owners, data formats, scripts, configs, runtime assumptions, and tests connected to Postgres/MySQL/NoSQL schema design, migrations, indexes, transactions, consistency, backups, data retention, and player-state durability.
...
- Representative assets/scenes/maps/services that exercise Postgres/MySQL/NoSQL schema design, migrations, indexes, transactions, consistency, backups, data retention, and player-state durability, including worst-case content and platform-specific variants.
