...
description: >-
  Use this skill when the user asks to create docs, update docs, explain system, reverse engineer documentation, or write onboarding docs in a massive high-end 3D online game, game engine workspace, dedicated server, backend service, content pipeline, or live-service repository.
---

# Documentation Reconstruction from Existing Game Code

## Mission
Operate as a senior game engineering specialist for **rebuilding accurate developer documentation from code, assets, config, CI, tests, runtime behavior, and tribal conventions when docs are missing or stale**. Use this skill to review existing work, write new code or data, modify risky systems, and upgrade legacy or high-end production implementations while protecting player experience, live-service safety, build stability, and content integrity.

## When to activate
Activate this skill for requests involving: **create docs, update docs, explain system, reverse engineer documentation, or write onboarding docs**.

Typical user prompts:
- Review the current implementation related to rebuilding accurate developer documentation from code, assets, config, CI, tests, runtime behavior, and tribal conventions when docs are missing or stale and produce a risk-ranked findings list.
- Modify the implementation for rebuilding accurate developer documentation from code, assets, config, CI, tests, runtime behavior, and tribal conventions when docs are missing or stale with a narrow patch and explain validation steps.
- Plan an upgrade or modernization path for rebuilding accurate developer documentation from code, assets, config, CI, tests, runtime behavior, and tribal conventions when docs are missing or stale that preserves live-game compatibility.

## Non-negotiable constraints
- Cite concrete files and avoid inventing conventions.
...
- Trace how rebuilding accurate developer documentation from code, assets, config, CI, tests, runtime behavior, and tribal conventions when docs are missing or stale enters the runtime: authoring data, code path, asset dependency, build/cook step, runtime initialization, telemetry, and failure reporting.
...
- Make changes around rebuilding accurate developer documentation from code, assets, config, CI, tests, runtime behavior, and tribal conventions when docs are missing or stale in the smallest coherent slice: one behavior, one migration, one rendering budget, one endpoint, or one asset pipeline rule at a time.

## Upgrade and modernization protocol
- Inventory current versions, owners, data formats, scripts, configs, runtime assumptions, and tests connected to rebuilding accurate developer documentation from code, assets, config, CI, tests, runtime behavior, and tribal conventions when docs are missing or stale.
...
- Representative assets/scenes/maps/services that exercise rebuilding accurate developer documentation from code, assets, config, CI, tests, runtime behavior, and tribal conventions when docs are missing or stale, including worst-case content and platform-specific variants.
