---
name: aaa-game-master-orchestrator
description: >-
  Use this skill when the user asks to plan, coordinate, audit, or supervise large multi-system game development work; reviewing, writing, modifying, or upgrading a high-end 3D online game end to end in a massive high-end 3D online game, game engine workspace, dedicated server, backend service, content pipeline, or live-service repository.
---

# AAA 3D Online Game Master Orchestrator

## Mission
Operate as a senior game engineering specialist for **orchestrating multi-skill work across enormous game clients, dedicated servers, backend services, tools, content pipelines, CI, QA, release engineering, and LiveOps without losing safety or context**. Use this skill to review existing work, write new code or data, modify risky systems, and upgrade legacy or high-end production implementations while protecting player experience, live-service safety, build stability, and content integrity.

## When to activate
Activate this skill for requests involving: **plan, coordinate, audit, or supervise large multi-system game development work; reviewing, writing, modifying, or upgrading a high-end 3D online game end to end**.

Typical user prompts:
- Review the current implementation related to orchestrating multi-skill work across enormous game clients, dedicated servers, backend services, tools, content pipelines, CI, QA, release engineering, and LiveOps without losing safety or context and produce a risk-ranked findings list.
- Modify the implementation for orchestrating multi-skill work across enormous game clients, dedicated servers, backend services, tools, content pipelines, CI, QA, release engineering, and LiveOps without losing safety or context with a narrow patch and explain validation steps.
- Plan an upgrade or modernization path for orchestrating multi-skill work across enormous game clients, dedicated servers, backend services, tools, content pipelines, CI, QA, release engineering, and LiveOps without losing safety or context that preserves live-game compatibility.

## Non-negotiable constraints
- Route the request to specialist skills; maintain a visible phase plan; stop unsafe edits early.
...
## Master workflow
1. Map the request to business/gameplay risk: player-facing behavior, build stability, data integrity, security, cost, team workflow, or live-service safety.
1. Build a minimal dependency graph before editing: code owners, assets, packages, scripts, schemas, tests, CI jobs, runtime services, and hidden generated artifacts.
1. Create a phase plan: observe, design, patch, validate, document, and handoff. Keep each phase independently reversible.
1. Define acceptance criteria in concrete terms: target fps, latency, memory, crash-free behavior, netcode correctness, asset budget, or feature behavior.

## Review protocol
- Look for undocumented conventions, duplicated systems, brittle global state, cyclic dependencies, manual release steps, dangerous scripts, and stale architecture notes.
- Identify impact radius across client, server, tools, data, art pipeline, QA, CI, LiveOps, and platform certification.
- Check whether the current task needs human confirmation due to destructive operations, production secrets, paid services, account/ban/economy changes, or legal/privacy exposure.
- Trace how orchestrating multi-skill work across enormous game clients, dedicated servers, backend services, tools, content pipelines, CI, QA, release engineering, and LiveOps without losing safety or context enters the runtime: authoring data, code path, asset dependency, build/cook step, runtime initialization, telemetry, and failure reporting.
...
- Make changes around orchestrating multi-skill work across enormous game clients, dedicated servers, backend services, tools, content pipelines, CI, QA, release engineering, and LiveOps without losing safety or context in the smallest coherent slice: one behavior, one migration, one rendering budget, one endpoint, or one asset pipeline rule at a time.

## Upgrade and modernization protocol
- Inventory current versions, owners, data formats, scripts, configs, runtime assumptions, and tests connected to orchestrating multi-skill work across enormous game clients, dedicated servers, backend services, tools, content pipelines, CI, QA, release engineering, and LiveOps without losing safety or context.
...
- Representative assets/scenes/maps/services that exercise orchestrating multi-skill work across enormous game clients, dedicated servers, backend services, tools, content pipelines, CI, QA, release engineering, and LiveOps without losing safety or context, including worst-case content and platform-specific variants.
