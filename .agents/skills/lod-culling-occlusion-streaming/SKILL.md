---
name: lod-culling-occlusion-streaming
description: >-
  Use this skill when the user asks to LOD, HLOD, culling, occlusion, impostor, billboard, streaming distance, visibility, overdraw, or crowd rendering in a massive high-end 3D online game, game engine workspace, dedicated server, backend service, content pipeline, or live-service repository.
---

# LOD, Culling, Occlusion, and Streaming

## Mission
Operate as a senior game engineering specialist for **mesh LODs, HLODs, occlusion culling, frustum culling, impostors, billboards, streaming distance, visibility cells, crowd culling, and overdraw reduction**. Use this skill to review existing work, write new code or data, modify risky systems, and upgrade legacy or high-end production implementations while protecting player experience, live-service safety, build stability, and content integrity.

## When to activate
Activate this skill for requests involving: **LOD, HLOD, culling, occlusion, impostor, billboard, streaming distance, visibility, overdraw, or crowd rendering**.

Typical user prompts:
- Review the current implementation related to mesh LODs, HLODs, occlusion culling, frustum culling, impostors, billboards, streaming distance, visibility cells, crowd culling, and overdraw reduction and produce a risk-ranked findings list.
- Modify the implementation for mesh LODs, HLODs, occlusion culling, frustum culling, impostors, billboards, streaming distance, visibility cells, crowd culling, and overdraw reduction with a narrow patch and explain validation steps.
- Plan an upgrade or modernization path for mesh LODs, HLODs, occlusion culling, frustum culling, impostors, billboards, streaming distance, visibility cells, crowd culling, and overdraw reduction that preserves live-game compatibility.

## Non-negotiable constraints
- Validate fast traversal, worst-case vistas, multiplayer density, and cinematic cameras.
...
- Trace how mesh LODs, HLODs, occlusion culling, frustum culling, impostors, billboards, streaming distance, visibility cells, crowd culling, and overdraw reduction enters the runtime: authoring data, code path, asset dependency, build/cook step, runtime initialization, telemetry, and failure reporting.
...
- Make changes around mesh LODs, HLODs, occlusion culling, frustum culling, impostors, billboards, streaming distance, visibility cells, crowd culling, and overdraw reduction in the smallest coherent slice: one behavior, one migration, one rendering budget, one endpoint, or one asset pipeline rule at a time.

## Upgrade and modernization protocol
- Inventory current versions, owners, data formats, scripts, configs, runtime assumptions, and tests connected to mesh LODs, HLODs, occlusion culling, frustum culling, impostors, billboards, streaming distance, visibility cells, crowd culling, and overdraw reduction.
...
- Representative assets/scenes/maps/services that exercise mesh LODs, HLODs, occlusion culling, frustum culling, impostors, billboards, streaming distance, visibility cells, crowd culling, and overdraw reduction, including worst-case content and platform-specific variants.
