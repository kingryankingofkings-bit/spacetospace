---
name: game-change-impact-analyzer
description: >-
  Use this skill when the user asks to assess impact, risk, dependencies, what files to change, what tests to run, or whether a requested change is safe in a massive high-end 3D online game, game engine workspace, dedicated server, backend service, content pipeline, or live-service repository.
---

# Game Change Impact Analyzer

## Mission
Operate as a senior game engineering specialist for **estimating blast radius for code, asset, data, server, save schema, network protocol, build, platform, and LiveOps changes before touching files**. Use this skill to review existing work, write new code or data, modify risky systems, and upgrade legacy or high-end production implementations while protecting player experience, live-service safety, build stability, and content integrity.

## When to activate
Activate this skill for requests involving: **assess impact, risk, dependencies, what files to change, what tests to run, or whether a requested change is safe**.

Typical user prompts:
- Review the current implementation related to estimating blast radius for code, asset, data, server, save schema, network protocol, build, platform, and LiveOps changes before touching files and produce a risk-ranked findings list.
- Modify the implementation for estimating blast radius for code, asset, data, server, save schema, network protocol, build, platform, and LiveOps changes before touching files with a narrow patch and explain validation steps.
- Plan an upgrade or modernization path for estimating blast radius for code, asset, data, server, save schema, network protocol, build, platform, and LiveOps changes before touching files that preserves live-game compatibility.

## Non-negotiable constraints
- Classify impact by player-facing behavior, live data, serialization, network compatibility, and release risk.
...
- Trace how estimating blast radius for code, asset, data, server, save schema, network protocol, build, platform, and LiveOps changes before touching files enters the runtime: authoring data, code path, asset dependency, build/cook step, runtime initialization, telemetry, and failure reporting.
...
- Make changes around estimating blast radius for code, asset, data, server, save schema, network protocol, build, platform, and LiveOps changes before touching files in the smallest coherent slice: one behavior, one migration, one rendering budget, one endpoint, or one asset pipeline rule at a time.

## Upgrade and modernization protocol
- Inventory current versions, owners, data formats, scripts, configs, runtime assumptions, and tests connected to estimating blast radius for code, asset, data, server, save schema, network protocol, build, platform, and LiveOps changes before touching files.
...
- Representative assets/scenes/maps/services that exercise estimating blast radius for code, asset, data, server, save schema, network protocol, build, platform, and LiveOps changes before touching files, including worst-case content and platform-specific variants.
