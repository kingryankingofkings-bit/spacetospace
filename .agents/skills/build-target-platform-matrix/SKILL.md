---
name: build-target-platform-matrix
description: >-
  Use this skill when the user asks to build matrix, target platform, client build, server build, development/shipping build, platform define, or quality tier in a massive high-end 3D online game, game engine workspace, dedicated server, backend service, content pipeline, or live-service repository.
---

# Build Target Platform Matrix

## Mission
Operate as a senior game engineering specialist for **managing build targets, configurations, defines, content variants, quality tiers, server/client separation, and platform-specific package requirements**. Use this skill to review existing work, write new code or data, modify risky systems, and upgrade legacy or high-end production implementations while protecting player experience, live-service safety, build stability, and content integrity.

## When to activate
Activate this skill for requests involving: **build matrix, target platform, client build, server build, development/shipping build, platform define, or quality tier**.

Typical user prompts:
- Review the current implementation related to managing build targets, configurations, defines, content variants, quality tiers, server/client separation, and platform-specific package requirements and produce a risk-ranked findings list.
- Modify the implementation for managing build targets, configurations, defines, content variants, quality tiers, server/client separation, and platform-specific package requirements with a narrow patch and explain validation steps.
- Plan an upgrade or modernization path for managing build targets, configurations, defines, content variants, quality tiers, server/client separation, and platform-specific package requirements that preserves live-game compatibility.

## Non-negotiable constraints
- Make every build target explicit with ownership, CI command, assets, and verification steps.
...
- Trace how managing build targets, configurations, defines, content variants, quality tiers, server/client separation, and platform-specific package requirements enters the runtime: authoring data, code path, asset dependency, build/cook step, runtime initialization, telemetry, and failure reporting.
...
- Make changes around managing build targets, configurations, defines, content variants, quality tiers, server/client separation, and platform-specific package requirements in the smallest coherent slice: one behavior, one migration, one rendering budget, one endpoint, or one asset pipeline rule at a time.

## Upgrade and modernization protocol
- Inventory current versions, owners, data formats, scripts, configs, runtime assumptions, and tests connected to managing build targets, configurations, defines, content variants, quality tiers, server/client separation, and platform-specific package requirements.
...
- Representative assets/scenes/maps/services that exercise managing build targets, configurations, defines, content variants, quality tiers, server/client separation, and platform-specific package requirements, including worst-case content and platform-specific variants.
