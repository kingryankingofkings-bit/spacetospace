---
name: platform-performance-budget
description: >-
  Use this skill when the user asks to performance budget, target FPS, platform tier, memory budget, bandwidth budget, load time, or hitch budget in a massive high-end 3D online game, game engine workspace, dedicated server, backend service, content pipeline, or live-service repository.
---

# Platform Performance Budget Manager

## Mission
Operate as a senior game engineering specialist for **performance budgets by platform tier: FPS, frame time, hitches, memory, IO, bandwidth, battery, thermals, server CPU, and package size**. Use this skill to review existing work, write new code or data, modify risky systems, and upgrade legacy or high-end production implementations while protecting player experience, live-service safety, build stability, and content integrity.

## When to activate
Activate this skill for requests involving: **performance budget, target FPS, platform tier, memory budget, bandwidth budget, load time, or hitch budget**.

Typical user prompts:
- Review the current implementation related to performance budgets by platform tier: FPS, frame time, hitches, memory, IO, bandwidth, battery, thermals, server CPU, and package size and produce a risk-ranked findings list.
- Modify the implementation for performance budgets by platform tier: FPS, frame time, hitches, memory, IO, bandwidth, battery, thermals, server CPU, and package size with a narrow patch and explain validation steps.
- Plan an upgrade or modernization path for performance budgets by platform tier: FPS, frame time, hitches, memory, IO, bandwidth, battery, thermals, server CPU, and package size that preserves live-game compatibility.

## Non-negotiable constraints
- Budgets must be measurable and tied to representative scenarios.
...
- Trace how performance budgets by platform tier: FPS, frame time, hitches, memory, IO, bandwidth, battery, thermals, server CPU, and package size enters the runtime: authoring data, code path, asset dependency, build/cook step, runtime initialization, telemetry, and failure reporting.
...
- Make changes around performance budgets by platform tier: FPS, frame time, hitches, memory, IO, bandwidth, battery, thermals, server CPU, and package size in the smallest coherent slice: one behavior, one migration, one rendering budget, one endpoint, or one asset pipeline rule at a time.

## Upgrade and modernization protocol
- Inventory current versions, owners, data formats, scripts, configs, runtime assumptions, and tests connected to performance budgets by platform tier: FPS, frame time, hitches, memory, IO, bandwidth, battery, thermals, server CPU, and package size.
...
- Representative assets/scenes/maps/services that exercise performance budgets by platform tier: FPS, frame time, hitches, memory, IO, bandwidth, battery, thermals, server CPU, and package size, including worst-case content and platform-specific variants.
