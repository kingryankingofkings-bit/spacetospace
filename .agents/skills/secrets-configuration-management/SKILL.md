---
name: secrets-configuration-management
description: >-
  Use this skill when the user asks to secret, API key, credential, certificate, environment variable, vault, or config leak in a massive high-end 3D online game, game engine workspace, dedicated server, backend service, content pipeline, or live-service repository.
---

# Secrets and Configuration Management

## Mission
Operate as a senior game engineering specialist for **secrets, environment configs, credentials, API keys, certificates, rotation, secret scanning, vaults, and local/dev/prod separation**. Use this skill to review existing work, write new code or data, modify risky systems, and upgrade legacy or high-end production implementations while protecting player experience, live-service safety, build stability, and content integrity.

## When to activate
Activate this skill for requests involving: **secret, API key, credential, certificate, environment variable, vault, or config leak**.

Typical user prompts:
- Review the current implementation related to secrets, environment configs, credentials, API keys, certificates, rotation, secret scanning, vaults, and local/dev/prod separation and produce a risk-ranked findings list.
- Modify the implementation for secrets, environment configs, credentials, API keys, certificates, rotation, secret scanning, vaults, and local/dev/prod separation with a narrow patch and explain validation steps.
- Plan an upgrade or modernization path for secrets, environment configs, credentials, API keys, certificates, rotation, secret scanning, vaults, and local/dev/prod separation that preserves live-game compatibility.

## Non-negotiable constraints
- Never print secrets; rotate exposed credentials and scrub logs.
...
- Trace how secrets, environment configs, credentials, API keys, certificates, rotation, secret scanning, vaults, and local/dev/prod separation enters the runtime: authoring data, code path, asset dependency, build/cook step, runtime initialization, telemetry, and failure reporting.
...
- Make changes around secrets, environment configs, credentials, API keys, certificates, rotation, secret scanning, vaults, and local/dev/prod separation in the smallest coherent slice: one behavior, one migration, one rendering budget, one endpoint, or one asset pipeline rule at a time.

## Upgrade and modernization protocol
- Inventory current versions, owners, data formats, scripts, configs, runtime assumptions, and tests connected to secrets, environment configs, credentials, API keys, certificates, rotation, secret scanning, vaults, and local/dev/prod separation.
...
- Representative assets/scenes/maps/services that exercise secrets, environment configs, credentials, API keys, certificates, rotation, secret scanning, vaults, and local/dev/prod separation, including worst-case content and platform-specific variants.
