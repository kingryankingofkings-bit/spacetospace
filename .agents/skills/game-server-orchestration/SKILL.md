---
name: game-server-orchestration
description: >-
  Use this skill when the user asks to game server fleet, orchestration, Agones, allocation, draining, rolling update, or fleet scaling in a massive high-end 3D online game, game engine workspace, dedicated server, backend service, content pipeline, or live-service repository.
---

# Game Server Orchestration

## Mission
Operate as a senior game engineering specialist for **allocating, scaling, draining, updating, and monitoring game server fleets across Kubernetes/Agones/custom fleets/managed providers**. Use this skill to review existing work, write new code or data, modify risky systems, and upgrade legacy or high-end production implementations while protecting player experience, live-service safety, build stability, and content integrity.

## When to activate
Activate this skill for requests involving: **game server fleet, orchestration, Agones, allocation, draining, rolling update, or fleet scaling**.

Typical user prompts:
- Review the current implementation related to allocating, scaling, draining, updating, and monitoring game server fleets across Kubernetes/Agones/custom fleets/managed providers and produce a risk-ranked findings list.
- Modify the implementation for allocating, scaling, draining, updating, and monitoring game server fleets across Kubernetes/Agones/custom fleets/managed providers with a narrow patch and explain validation steps.
- Plan an upgrade or modernization path for allocating, scaling, draining, updating, and monitoring game server fleets across Kubernetes/Agones/custom fleets/managed providers that preserves live-game compatibility.

## Non-negotiable constraints
- Protect active matches during deploys and instance shutdown.
...
- Trace how allocating, scaling, draining, updating, and monitoring game server fleets across Kubernetes/Agones/custom fleets/managed providers enters the runtime: authoring data, code path, asset dependency, build/cook step, runtime initialization, telemetry, and failure reporting.
...
- Make changes around allocating, scaling, draining, updating, and monitoring game server fleets across Kubernetes/Agones/custom fleets/managed providers in the smallest coherent slice: one behavior, one migration, one rendering budget, one endpoint, or one asset pipeline rule at a time.

## Upgrade and modernization protocol
- Inventory current versions, owners, data formats, scripts, configs, runtime assumptions, and tests connected to allocating, scaling, draining, updating, and monitoring game server fleets across Kubernetes/Agones/custom fleets/managed providers.
...
- Representative assets/scenes/maps/services that exercise allocating, scaling, draining, updating, and monitoring game server fleets across Kubernetes/Agones/custom fleets/managed providers, including worst-case content and platform-specific variants.
