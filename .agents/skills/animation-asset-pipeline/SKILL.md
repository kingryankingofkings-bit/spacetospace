---
name: animation-asset-pipeline
description: >-
  Use this skill when the user asks to animation import, retargeting, skeleton, root motion, mocap, additive animation, animation compression, or notifies in a massive high-end 3D online game, game engine workspace, dedicated server, backend service, content pipeline, or live-service repository.
---

# Animation Asset Pipeline

## Mission
Operate as a senior game engineering specialist for **animation import/export, retargeting, skeleton compatibility, root motion data, compression, additive clips, mocap cleanup, state machine assets, and events/notifies**. Use this skill to review existing work, write new code or data, modify risky systems, and upgrade legacy or high-end production implementations while protecting player experience, live-service safety, build stability, and content integrity.

## When to activate
Activate this skill for requests involving: **animation import, retargeting, skeleton, root motion, mocap, additive animation, animation compression, or notifies**.

Typical user prompts:
- Review the current implementation related to animation import/export, retargeting, skeleton compatibility, root motion data, compression, additive clips, mocap cleanup, state machine assets, and events/notifies and produce a risk-ranked findings list.
- Modify the implementation for animation import/export, retargeting, skeleton compatibility, root motion data, compression, additive clips, mocap cleanup, state machine assets, and events/notifies with a narrow patch and explain validation steps.
- Plan an upgrade or modernization path for animation import/export, retargeting, skeleton compatibility, root motion data, compression, additive clips, mocap cleanup, state machine assets, and events/notifies that preserves live-game compatibility.

## Non-negotiable constraints
- Protect gameplay-critical notifies/root motion while optimizing compression.
...
## Master workflow
1. Map the asset path from DCC/source art to engine import, metadata, validation, cooking/bundling, CDN/package deployment, runtime streaming, and patch rollback.
1. Treat binary assets as code: source control locks where needed, deterministic imports, automated validation, naming conventions, dependency checks, and budget gates.
1. For open worlds, validate cells/chunks/levels by traversal path, teleport path, worst-case fast travel, multiplayer replication zones, and memory pressure.
1. Design content changes to be incremental and patch-friendly; avoid invalidating huge bundles unless there is a deliberate release reason.

## Review protocol
- Inspect GUID/meta stability, circular asset dependencies, missing source files, duplicate textures/materials, naming drift, oversized bundles, and address/path fragility.
- Check streaming hitches, shader compilation stalls, missing LODs, collision complexity, navmesh generation, audio/VFX residency, and platform texture formats.
- Confirm that editor tools, validators, import settings, and artist documentation match actual CI/cook/build behavior.
- Trace how animation import/export, retargeting, skeleton compatibility, root motion data, compression, additive clips, mocap cleanup, state machine assets, and events/notifies enters the runtime: authoring data, code path, asset dependency, build/cook step, runtime initialization, telemetry, and failure reporting.
...
- Make changes around animation import/export, retargeting, skeleton compatibility, root motion data, compression, additive clips, mocap cleanup, state machine assets, and events/notifies in the smallest coherent slice: one behavior, one migration, one rendering budget, one endpoint, or one asset pipeline rule at a time.

## Upgrade and modernization protocol
- Inventory current versions, owners, data formats, scripts, configs, runtime assumptions, and tests connected to animation import/export, retargeting, skeleton compatibility, root motion data, compression, additive clips, mocap cleanup, state machine assets, and events/notifies.
...
- Representative assets/scenes/maps/services that exercise animation import/export, retargeting, skeleton compatibility, root motion data, compression, additive clips, mocap cleanup, state machine assets, and events/notifies, including worst-case content and platform-specific variants.
