---
name: ui-rendering-compositing
description: >-
  Use this skill when the user asks to UI rendering, HUD performance, canvas, widget, render texture, safe area, DPI, localization overflow, or UI compositing in a massive high-end 3D online game, game engine workspace, dedicated server, backend service, content pipeline, or live-service repository.
---

# UI Rendering and Compositing

## Mission
Operate as a senior game engineering specialist for **UI draw calls, canvases, widgets, render textures, safe areas, DPI scaling, localization overflow, HDR UI, diegetic UI, input focus, and performance**. Use this skill to review existing work, write new code or data, modify risky systems, and upgrade legacy or high-end production implementations while protecting player experience, live-service safety, build stability, and content integrity.

## When to activate
Activate this skill for requests involving: **UI rendering, HUD performance, canvas, widget, render texture, safe area, DPI, localization overflow, or UI compositing**.

Typical user prompts:
- Review the current implementation related to UI draw calls, canvases, widgets, render textures, safe areas, DPI scaling, localization overflow, HDR UI, diegetic UI, input focus, and performance and produce a risk-ranked findings list.
- Modify the implementation for UI draw calls, canvases, widgets, render textures, safe areas, DPI scaling, localization overflow, HDR UI, diegetic UI, input focus, and performance with a narrow patch and explain validation steps.
- Plan an upgrade or modernization path for UI draw calls, canvases, widgets, render textures, safe areas, DPI scaling, localization overflow, HDR UI, diegetic UI, input focus, and performance that preserves live-game compatibility.

## Non-negotiable constraints
- Protect readability, accessibility, localization, and input modes while optimizing.
...
- Trace how UI draw calls, canvases, widgets, render textures, safe areas, DPI scaling, localization overflow, HDR UI, diegetic UI, input focus, and performance enters the runtime: authoring data, code path, asset dependency, build/cook step, runtime initialization, telemetry, and failure reporting.
...
- Make changes around UI draw calls, canvases, widgets, render textures, safe areas, DPI scaling, localization overflow, HDR UI, diegetic UI, input focus, and performance in the smallest coherent slice: one behavior, one migration, one rendering budget, one endpoint, or one asset pipeline rule at a time.

## Upgrade and modernization protocol
- Inventory current versions, owners, data formats, scripts, configs, runtime assumptions, and tests connected to UI draw calls, canvases, widgets, render textures, safe areas, DPI scaling, localization overflow, HDR UI, diegetic UI, input focus, and performance.
...
- Representative assets/scenes/maps/services that exercise UI draw calls, canvases, widgets, render textures, safe areas, DPI scaling, localization overflow, HDR UI, diegetic UI, input focus, and performance, including worst-case content and platform-specific variants.
