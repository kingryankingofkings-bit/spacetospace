---
name: machine-learning-gameplay-tools
description: >-
  Use this skill when the user asks to machine learning, ML bot, AI assistant, procedural ML, personalization, moderation model, or QA bot in a massive high-end 3D online game, game engine workspace, dedicated server, backend service, content pipeline, or live-service repository.
---

# Machine Learning Gameplay Tools

## Mission
Operate as a senior game engineering specialist for **using ML safely for bots, procedural content, animation, moderation assistance, personalization, QA bots, and developer tooling without compromising determinism or fairness**. Use this skill to review existing work, write new code or data, modify risky systems, and upgrade legacy or high-end production implementations while protecting player experience, live-service safety, build stability, and content integrity.

## When to activate
Activate this skill for requests involving: **machine learning, ML bot, AI assistant, procedural ML, personalization, moderation model, or QA bot**.

Typical user prompts:
- Review the current implementation related to using ML safely for bots, procedural content, animation, moderation assistance, personalization, QA bots, and developer tooling without compromising determinism or fairness and produce a risk-ranked findings list.
- Modify the implementation for using ML safely for bots, procedural content, animation, moderation assistance, personalization, QA bots, and developer tooling without compromising determinism or fairness with a narrow patch and explain validation steps.
- Plan an upgrade or modernization path for using ML safely for bots, procedural content, animation, moderation assistance, personalization, QA bots, and developer tooling without compromising determinism or fairness that preserves live-game compatibility.

## Non-negotiable constraints
- Avoid opaque critical gameplay decisions unless explainability and fairness controls exist.
...
## Master workflow
1. Start with player-facing intent, then define domain invariants: authority, cooldowns, resource costs, state transitions, animation timing, input windows, and failure cases.
1. Separate deterministic simulation from presentation, VFX/SFX, UI, telemetry, and editor tooling.
1. Use data-driven assets/configs for tunable values, but keep validation, permissions, and critical game rules in trusted code paths.
1. When changing feel-heavy systems, preserve replayable tests plus human playtest notes; do not rely only on compile success.

## Review protocol
- Inspect tick order, event lifetimes, object pooling, save/load implications, animation/gameplay sync, physics determinism, null state transitions, and designer-authored data validity.
- Look for client-trusted gameplay decisions, unbounded timers, duplicated state, stale buffs/effects, bad rollback interactions, and exploit-prone economy hooks.
- Validate edge cases: pause, disconnect, respawn, fast travel, spectating, death, inventory full, quest re-entry, level streaming, localization, accessibility, and frame-rate variance.
- Trace how using ML safely for bots, procedural content, animation, moderation assistance, personalization, QA bots, and developer tooling without compromising determinism or fairness enters the runtime: authoring data, code path, asset dependency, build/cook step, runtime initialization, telemetry, and failure reporting.
...
- Make changes around using ML safely for bots, procedural content, animation, moderation assistance, personalization, QA bots, and developer tooling without compromising determinism or fairness in the smallest coherent slice: one behavior, one migration, one rendering budget, one endpoint, or one asset pipeline rule at a time.

## Upgrade and modernization protocol
- Inventory current versions, owners, data formats, scripts, configs, runtime assumptions, and tests connected to using ML safely for bots, procedural content, animation, moderation assistance, personalization, QA bots, and developer tooling without compromising determinism or fairness.
...
- Representative assets/scenes/maps/services that exercise using ML safely for bots, procedural content, animation, moderation assistance, personalization, QA bots, and developer tooling without compromising determinism or fairness, including worst-case content and platform-specific variants.
