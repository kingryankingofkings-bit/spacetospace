---
name: large-asset-version-control
description: >-
  Use this skill when the user asks to version control, Git LFS, Perforce, Unity Version Control, binary assets, file locks, merge conflicts, repo size, or source control setup in a massive high-end 3D online game, game engine workspace, dedicated server, backend service, content pipeline, or live-service repository.
---

# Large Asset Version Control and Locking

## Mission
Operate as a senior game engineering specialist for **source-control workflows for huge binary game assets, Unreal uassets, Unity scenes/prefabs/meta files, Godot resources, Git LFS, Perforce, locks, branches, and merges**. Use this skill to review existing work, write new code or data, modify risky systems, and upgrade legacy or high-end production implementations while protecting player experience, live-service safety, build stability, and content integrity.

## When to activate
Activate this skill for requests involving: **version control, Git LFS, Perforce, Unity Version Control, binary assets, file locks, merge conflicts, repo size, or source control setup**.

Typical user prompts:
- Review the current implementation related to source-control workflows for huge binary game assets, Unreal uassets, Unity scenes/prefabs/meta files, Godot resources, Git LFS, Perforce, locks, branches, and merges and produce a risk-ranked findings list.
- Modify the implementation for source-control workflows for huge binary game assets, Unreal uassets, Unity scenes/prefabs/meta files, Godot resources, Git LFS, Perforce, locks, branches, and merges with a narrow patch and explain validation steps.
- Plan an upgrade or modernization path for source-control workflows for huge binary game assets, Unreal uassets, Unity scenes/prefabs/meta files, Godot resources, Git LFS, Perforce, locks, branches, and merges that preserves live-game compatibility.

## Non-negotiable constraints
- Protect binary assets, meta/GUID stability, locks, and reproducible history.
...
- Trace how source-control workflows for huge binary game assets, Unreal uassets, Unity scenes/prefabs/meta files, Godot resources, Git LFS, Perforce, locks, branches, and merges enters the runtime: authoring data, code path, asset dependency, build/cook step, runtime initialization, telemetry, and failure reporting.
...
- Make changes around source-control workflows for huge binary game assets, Unreal uassets, Unity scenes/prefabs/meta files, Godot resources, Git LFS, Perforce, locks, branches, and merges in the smallest coherent slice: one behavior, one migration, one rendering budget, one endpoint, or one asset pipeline rule at a time.

## Upgrade and modernization protocol
- Inventory current versions, owners, data formats, scripts, configs, runtime assumptions, and tests connected to source-control workflows for huge binary game assets, Unreal uassets, Unity scenes/prefabs/meta files, Godot resources, Git LFS, Perforce, locks, branches, and merges.
...
- Representative assets/scenes/maps/services that exercise source-control workflows for huge binary game assets, Unreal uassets, Unity scenes/prefabs/meta files, Godot resources, Git LFS, Perforce, locks, branches, and merges, including worst-case content and platform-specific variants.
