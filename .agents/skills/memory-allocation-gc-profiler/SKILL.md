---
name: memory-allocation-gc-profiler
description: >-
  Use this skill when the user asks to memory, GC, allocation, leak, object pool, native memory, memory snapshot, or out of memory in a massive high-end 3D online game, game engine workspace, dedicated server, backend service, content pipeline, or live-service repository.
---

# Memory Allocation and GC Profiler

## Mission
Operate as a senior game engineering specialist for **memory leaks, allocation churn, garbage collection, object pooling, asset residency, fragmentation, snapshots, native memory, and platform budgets**. Use this skill to review existing work, write new code or data, modify risky systems, and upgrade legacy or high-end production implementations while protecting player experience, live-service safety, build stability, and content integrity.

## When to activate
Activate this skill for requests involving: **memory, GC, allocation, leak, object pool, native memory, memory snapshot, or out of memory**.

Typical user prompts:
- Review the current implementation related to memory leaks, allocation churn, garbage collection, object pooling, asset residency, fragmentation, snapshots, native memory, and platform budgets and produce a risk-ranked findings list.
- Modify the implementation for memory leaks, allocation churn, garbage collection, object pooling, asset residency, fragmentation, snapshots, native memory, and platform budgets with a narrow patch and explain validation steps.
- Plan an upgrade or modernization path for memory leaks, allocation churn, garbage collection, object pooling, asset residency, fragmentation, snapshots, native memory, and platform budgets that preserves live-game compatibility.

## Non-negotiable constraints
- Profile allocations and ownership before pooling everything.
...
- Trace how memory leaks, allocation churn, garbage collection, object pooling, asset residency, fragmentation, snapshots, native memory, and platform budgets enters the runtime: authoring data, code path, asset dependency, build/cook step, runtime initialization, telemetry, and failure reporting.
...
- Make changes around memory leaks, allocation churn, garbage collection, object pooling, asset residency, fragmentation, snapshots, native memory, and platform budgets in the smallest coherent slice: one behavior, one migration, one rendering budget, one endpoint, or one asset pipeline rule at a time.

## Upgrade and modernization protocol
- Inventory current versions, owners, data formats, scripts, configs, runtime assumptions, and tests connected to memory leaks, allocation churn, garbage collection, object pooling, asset residency, fragmentation, snapshots, native memory, and platform budgets.
...
- Representative assets/scenes/maps/services that exercise memory leaks, allocation churn, garbage collection, object pooling, asset residency, fragmentation, snapshots, native memory, and platform budgets, including worst-case content and platform-specific variants.
