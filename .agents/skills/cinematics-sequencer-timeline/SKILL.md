---
name: cinematics-sequencer-timeline
description: >-
  Use this skill when the user asks to cinematic, cutscene, Sequencer, Timeline, camera cut, dialogue timing, or scripted sequence in a massive high-end 3D online game, game engine workspace, dedicated server, backend service, content pipeline, or live-service repository.
---

# Cinematics, Sequencer, Timeline, and Cutscenes

## Mission
Operate as a senior game engineering specialist for **cinematic tools, Unreal Sequencer, Unity Timeline/Cinemachine, animation tracks, camera cuts, localization timing, multiplayer gating, and skip/resume behavior**. Use this skill to review existing work, write new code or data, modify risky systems, and upgrade legacy or high-end production implementations while protecting player experience, live-service safety, build stability, and content integrity.

## When to activate
Activate this skill for requests involving: **cinematic, cutscene, Sequencer, Timeline, camera cut, dialogue timing, or scripted sequence**.

Typical user prompts:
- Review the current implementation related to cinematic tools, Unreal Sequencer, Unity Timeline/Cinemachine, animation tracks, camera cuts, localization timing, multiplayer gating, and skip/resume behavior and produce a risk-ranked findings list.
- Modify the implementation for cinematic tools, Unreal Sequencer, Unity Timeline/Cinemachine, animation tracks, camera cuts, localization timing, multiplayer gating, and skip/resume behavior with a narrow patch and explain validation steps.
- Plan an upgrade or modernization path for cinematic tools, Unreal Sequencer, Unity Timeline/Cinemachine, animation tracks, camera cuts, localization timing, multiplayer gating, and skip/resume behavior that preserves live-game compatibility.

## Non-negotiable constraints
- Validate skip, pause, disconnect, localization length, and streaming preload.
...
- Trace how cinematic tools, Unreal Sequencer, Unity Timeline/Cinemachine, animation tracks, camera cuts, localization timing, multiplayer gating, and skip/resume behavior enters the runtime: authoring data, code path, asset dependency, build/cook step, runtime initialization, telemetry, and failure reporting.
...
- Make changes around cinematic tools, Unreal Sequencer, Unity Timeline/Cinemachine, animation tracks, camera cuts, localization timing, multiplayer gating, and skip/resume behavior in the smallest coherent slice: one behavior, one migration, one rendering budget, one endpoint, or one asset pipeline rule at a time.

## Upgrade and modernization protocol
- Inventory current versions, owners, data formats, scripts, configs, runtime assumptions, and tests connected to cinematic tools, Unreal Sequencer, Unity Timeline/Cinemachine, animation tracks, camera cuts, localization timing, multiplayer gating, and skip/resume behavior.
...
- Representative assets/scenes/maps/services that exercise cinematic tools, Unreal Sequencer, Unity Timeline/Cinemachine, animation tracks, camera cuts, localization timing, multiplayer gating, and skip/resume behavior, including worst-case content and platform-specific variants.
