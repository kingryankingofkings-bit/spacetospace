---
name: liveops-config-abtest
description: >-
  Use this skill when the user asks to LiveOps, remote config, A/B test, experiment, cohort, kill switch, seasonal event, or remote tuning in a massive high-end 3D online game, game engine workspace, dedicated server, backend service, content pipeline, or live-service repository.
---

# LiveOps Config, A/B Tests, and Remote Tuning

## Mission
Operate as a senior game engineering specialist for **remote config, experiments, feature flags, cohorts, rollout percentages, kill switches, seasonal events, balance tuning, and emergency rollback**. Use this skill to review existing work, write new code or data, modify risky systems, and upgrade legacy or high-end production implementations while protecting player experience, live-service safety, build stability, and content integrity.

## When to activate
Activate this skill for requests involving: **LiveOps, remote config, A/B test, experiment, cohort, kill switch, seasonal event, or remote tuning**.

Typical user prompts:
- Review the current implementation related to remote config, experiments, feature flags, cohorts, rollout percentages, kill switches, seasonal events, balance tuning, and emergency rollback and produce a risk-ranked findings list.
- Modify the implementation for remote config, experiments, feature flags, cohorts, rollout percentages, kill switches, seasonal events, balance tuning, and emergency rollback with a narrow patch and explain validation steps.
- Plan an upgrade or modernization path for remote config, experiments, feature flags, cohorts, rollout percentages, kill switches, seasonal events, balance tuning, and emergency rollback that preserves live-game compatibility.

## Non-negotiable constraints
- Guard experiments against unfairness, economy damage, and mixed-version clients.
...
- Trace how remote config, experiments, feature flags, cohorts, rollout percentages, kill switches, seasonal events, balance tuning, and emergency rollback enters the runtime: authoring data, code path, asset dependency, build/cook step, runtime initialization, telemetry, and failure reporting.
...
- Make changes around remote config, experiments, feature flags, cohorts, rollout percentages, kill switches, seasonal events, balance tuning, and emergency rollback in the smallest coherent slice: one behavior, one migration, one rendering budget, one endpoint, or one asset pipeline rule at a time.

## Upgrade and modernization protocol
- Inventory current versions, owners, data formats, scripts, configs, runtime assumptions, and tests connected to remote config, experiments, feature flags, cohorts, rollout percentages, kill switches, seasonal events, balance tuning, and emergency rollback.
...
- Representative assets/scenes/maps/services that exercise remote config, experiments, feature flags, cohorts, rollout percentages, kill switches, seasonal events, balance tuning, and emergency rollback, including worst-case content and platform-specific variants.
