---
name: gameplay-framework-boundaries
description: >-
  Use this skill when the user asks to architecture boundary, gameplay layer, domain model, service layer, decouple, modularize, or clean game architecture in a massive high-end 3D online game, game engine workspace, dedicated server, backend service, content pipeline, or live-service repository.
---

# Gameplay Framework Boundaries

## Mission
Operate as a senior game engineering specialist for **clean boundaries between gameplay domain logic, engine presentation, networking, persistence, UI, tools, and analytics in large teams**. Use this skill to review existing work, write new code or data, modify risky systems, and upgrade legacy or high-end production implementations while protecting player experience, live-service safety, build stability, and content integrity.

## When to activate
Activate this skill for requests involving: **architecture boundary, gameplay layer, domain model, service layer, decouple, modularize, or clean game architecture**.

Typical user prompts:
- Review the current implementation related to clean boundaries between gameplay domain logic, engine presentation, networking, persistence, UI, tools, and analytics in large teams and produce a risk-ranked findings list.
- Modify the implementation for clean boundaries between gameplay domain logic, engine presentation, networking, persistence, UI, tools, and analytics in large teams with a narrow patch and explain validation steps.
- Plan an upgrade or modernization path for clean boundaries between gameplay domain logic, engine presentation, networking, persistence, UI, tools, and analytics in large teams that preserves live-game compatibility.

## Non-negotiable constraints
- Define ownership and dependency direction; avoid gameplay rules hidden in UI/VFX/tools.
...
- Trace how clean boundaries between gameplay domain logic, engine presentation, networking, persistence, UI, tools, and analytics in large teams enters the runtime: authoring data, code path, asset dependency, build/cook step, runtime initialization, telemetry, and failure reporting.
...
- Make changes around clean boundaries between gameplay domain logic, engine presentation, networking, persistence, UI, tools, and analytics in large teams in the smallest coherent slice: one behavior, one migration, one rendering budget, one endpoint, or one asset pipeline rule at a time.

## Upgrade and modernization protocol
- Inventory current versions, owners, data formats, scripts, configs, runtime assumptions, and tests connected to clean boundaries between gameplay domain logic, engine presentation, networking, persistence, UI, tools, and analytics in large teams.
...
- Representative assets/scenes/maps/services that exercise clean boundaries between gameplay domain logic, engine presentation, networking, persistence, UI, tools, and analytics in large teams, including worst-case content and platform-specific variants.
