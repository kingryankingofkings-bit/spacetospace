...
description: >-
  Use this skill when the user asks to feature flag, config, remote config, console variable, build define, quality setting, experiment, or toggle in a massive high-end 3D online game, game engine workspace, dedicated server, backend service, content pipeline, or live-service repository.
---

# Configuration and Feature Flags

## Mission
Operate as a senior game engineering specialist for **safe runtime configuration, feature flags, cvars, console commands, remote config, build defines, quality tiers, and experiment gates**. Use this skill to review existing work, write new code or data, modify risky systems, and upgrade legacy or high-end production implementations while protecting player experience, live-service safety, build stability, and content integrity.

## When to activate
Activate this skill for requests involving: **feature flag, config, remote config, console variable, build define, quality setting, experiment, or toggle**.

Typical user prompts:
- Review the current implementation related to safe runtime configuration, feature flags, cvars, console commands, remote config, build defines, quality tiers, and experiment gates and produce a risk-ranked findings list.
- Modify the implementation for safe runtime configuration, feature flags, cvars, console commands, remote config, build defines, quality tiers, and experiment gates with a narrow patch and explain validation steps.
- Plan an upgrade or modernization path for safe runtime configuration, feature flags, cvars, console commands, remote config, build defines, quality tiers, and experiment gates that preserves live-game compatibility.

## Non-negotiable constraints
- Protect server-authoritative flags, staged rollout, default values, and compatibility across client versions.
...
- Trace how safe runtime configuration, feature flags, cvars, console commands, remote config, build defines, quality tiers, and experiment gates enters the runtime: authoring data, code path, asset dependency, build/cook step, runtime initialization, telemetry, and failure reporting.
...
- Make changes around safe runtime configuration, feature flags, cvars, console commands, remote config, build defines, quality tiers, and experiment gates in the smallest coherent slice: one behavior, one migration, one rendering budget, one endpoint, or one asset pipeline rule at a time.

## Upgrade and modernization protocol
- Inventory current versions, owners, data formats, scripts, configs, runtime assumptions, and tests connected to safe runtime configuration, feature flags, cvars, console commands, remote config, build defines, quality tiers, and experiment gates.
...
- Representative assets/scenes/maps/services that exercise safe runtime configuration, feature flags, cvars, console commands, remote config, build defines, quality tiers, and experiment gates, including worst-case content and platform-specific variants.
