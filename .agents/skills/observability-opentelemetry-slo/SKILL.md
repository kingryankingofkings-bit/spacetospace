---
name: observability-opentelemetry-slo
description: >-
  Use this skill when the user asks to OpenTelemetry, observability, tracing, metrics, logs, SLO, dashboard, alert, or correlation ID in a massive high-end 3D online game, game engine workspace, dedicated server, backend service, content pipeline, or live-service repository.
---

# Observability, OpenTelemetry, and SLOs

## Mission
Operate as a senior game engineering specialist for **distributed tracing, metrics, logs, correlation IDs, client/server spans, dashboards, alerts, service-level objectives, and incident diagnosis**. Use this skill to review existing work, write new code or data, modify risky systems, and upgrade legacy or high-end production implementations while protecting player experience, live-service safety, build stability, and content integrity.

## When to activate
Activate this skill for requests involving: **OpenTelemetry, observability, tracing, metrics, logs, SLO, dashboard, alert, or correlation ID**.

Typical user prompts:
- Review the current implementation related to distributed tracing, metrics, logs, correlation IDs, client/server spans, dashboards, alerts, service-level objectives, and incident diagnosis and produce a risk-ranked findings list.
- Modify the implementation for distributed tracing, metrics, logs, correlation IDs, client/server spans, dashboards, alerts, service-level objectives, and incident diagnosis with a narrow patch and explain validation steps.
- Plan an upgrade or modernization path for distributed tracing, metrics, logs, correlation IDs, client/server spans, dashboards, alerts, service-level objectives, and incident diagnosis that preserves live-game compatibility.

## Non-negotiable constraints
- Instrument before optimizing; alert on player impact, not vanity metrics.
...
- Trace how distributed tracing, metrics, logs, correlation IDs, client/server spans, dashboards, alerts, service-level objectives, and incident diagnosis enters the runtime: authoring data, code path, asset dependency, build/cook step, runtime initialization, telemetry, and failure reporting.
...
- Make changes around distributed tracing, metrics, logs, correlation IDs, client/server spans, dashboards, alerts, service-level objectives, and incident diagnosis in the smallest coherent slice: one behavior, one migration, one rendering budget, one endpoint, or one asset pipeline rule at a time.

## Upgrade and modernization protocol
- Inventory current versions, owners, data formats, scripts, configs, runtime assumptions, and tests connected to distributed tracing, metrics, logs, correlation IDs, client/server spans, dashboards, alerts, service-level objectives, and incident diagnosis.
...
- Representative assets/scenes/maps/services that exercise distributed tracing, metrics, logs, correlation IDs, client/server spans, dashboards, alerts, service-level objectives, and incident diagnosis, including worst-case content and platform-specific variants.
