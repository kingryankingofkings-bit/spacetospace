---
name: performance-forensics-war-room
description: >-
  Use this skill when the user asks to performance emergency, fps drop, hitch, memory spike, server overload, launch blocker, or optimization war room in a massive high-end 3D online game, game engine workspace, dedicated server, backend service, content pipeline, or live-service repository.
---

# Performance Forensics War Room

## Mission
Operate as a senior game engineering specialist for **rapidly investigating severe FPS drops, hitches, memory spikes, server CPU overload, packet spikes, IO stalls, shader stalls, and launch blockers**. Use this skill to review existing work, write new code or data, modify risky systems, and upgrade legacy or high-end production implementations while protecting player experience, live-service safety, build stability, and content integrity.

## When to activate
Activate this skill for requests involving: **performance emergency, fps drop, hitch, memory spike, server overload, launch blocker, or optimization war room**.

Typical user prompts:
- Review the current implementation related to rapidly investigating severe FPS drops, hitches, memory spikes, server CPU overload, packet spikes, IO stalls, shader stalls, and launch blockers and produce a risk-ranked findings list.
- Modify the implementation for rapidly investigating severe FPS drops, hitches, memory spikes, server CPU overload, packet spikes, IO stalls, shader stalls, and launch blockers with a narrow patch and explain validation steps.
- Plan an upgrade or modernization path for rapidly investigating severe FPS drops, hitches, memory spikes, server CPU overload, packet spikes, IO stalls, shader stalls, and launch blockers that preserves live-game compatibility.

## Non-negotiable constraints
- Stabilize reproduction, isolate bottleneck, apply smallest safe fix, and capture before/after evidence.
...
- Trace how rapidly investigating severe FPS drops, hitches, memory spikes, server CPU overload, packet spikes, IO stalls, shader stalls, and launch blockers enters the runtime: authoring data, code path, asset dependency, build/cook step, runtime initialization, telemetry, and failure reporting.
...
- Make changes around rapidly investigating severe FPS drops, hitches, memory spikes, server CPU overload, packet spikes, IO stalls, shader stalls, and launch blockers in the smallest coherent slice: one behavior, one migration, one rendering budget, one endpoint, or one asset pipeline rule at a time.

## Upgrade and modernization protocol
- Inventory current versions, owners, data formats, scripts, configs, runtime assumptions, and tests connected to rapidly investigating severe FPS drops, hitches, memory spikes, server CPU overload, packet spikes, IO stalls, shader stalls, and launch blockers.
...
- Representative assets/scenes/maps/services that exercise rapidly investigating severe FPS drops, hitches, memory spikes, server CPU overload, packet spikes, IO stalls, shader stalls, and launch blockers, including worst-case content and platform-specific variants.
