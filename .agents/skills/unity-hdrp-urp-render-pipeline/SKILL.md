---
name: unity-hdrp-urp-render-pipeline
description: >-
  Use this skill when the user asks to Unity HDRP, URP, SRP, renderer feature, SRP batcher, camera stack, post processing, or Unity graphics settings in a massive high-end 3D online game, game engine workspace, dedicated server, backend service, content pipeline, or live-service repository.
---

# Unity HDRP/URP Render Pipeline Specialist

## Mission
Operate as a senior game engineering specialist for **Unity URP/HDRP setup, renderer features, SRP batcher, shader variants, lighting modes, quality profiles, camera stacks, post-process volumes, and platform tiers**. Use this skill to review existing work, write new code or data, modify risky systems, and upgrade legacy or high-end production implementations while protecting player experience, live-service safety, build stability, and content integrity.

## When to activate
Activate this skill for requests involving: **Unity HDRP, URP, SRP, renderer feature, SRP batcher, camera stack, post processing, or Unity graphics settings**.

Typical user prompts:
- Review the current implementation related to Unity URP/HDRP setup, renderer features, SRP batcher, shader variants, lighting modes, quality profiles, camera stacks, post-process volumes, and platform tiers and produce a risk-ranked findings list.
- Modify the implementation for Unity URP/HDRP setup, renderer features, SRP batcher, shader variants, lighting modes, quality profiles, camera stacks, post-process volumes, and platform tiers with a narrow patch and explain validation steps.
- Plan an upgrade or modernization path for Unity URP/HDRP setup, renderer features, SRP batcher, shader variants, lighting modes, quality profiles, camera stacks, post-process volumes, and platform tiers that preserves live-game compatibility.

## Non-negotiable constraints
- Align pipeline choice with target platforms and content budgets.
...
- Trace how Unity URP/HDRP setup, renderer features, SRP batcher, shader variants, lighting modes, quality profiles, camera stacks, post-process volumes, and platform tiers enters the runtime: authoring data, code path, asset dependency, build/cook step, runtime initialization, telemetry, and failure reporting.
...
- Make changes around Unity URP/HDRP setup, renderer features, SRP batcher, shader variants, lighting modes, quality profiles, camera stacks, post-process volumes, and platform tiers in the smallest coherent slice: one behavior, one migration, one rendering budget, one endpoint, or one asset pipeline rule at a time.

## Upgrade and modernization protocol
- Inventory current versions, owners, data formats, scripts, configs, runtime assumptions, and tests connected to Unity URP/HDRP setup, renderer features, SRP batcher, shader variants, lighting modes, quality profiles, camera stacks, post-process volumes, and platform tiers.
...
- Representative assets/scenes/maps/services that exercise Unity URP/HDRP setup, renderer features, SRP batcher, shader variants, lighting modes, quality profiles, camera stacks, post-process volumes, and platform tiers, including worst-case content and platform-specific variants.
