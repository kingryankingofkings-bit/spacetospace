---
name: disconnect-reconnect-session-recovery
description: >-
  Use this skill when the user asks to disconnect, reconnect, rejoin, timeout, session recovery, server crash, or host migration in a massive high-end 3D online game, game engine workspace, dedicated server, backend service, content pipeline, or live-service repository.
---

# Disconnect, Reconnect, and Session Recovery

## Mission
Operate as a senior game engineering specialist for **handling disconnects, reconnect, rejoin, timeout, host migration where applicable, server crash recovery, save handoff, and match integrity**. Use this skill to review existing work, write new code or data, modify risky systems, and upgrade legacy or high-end production implementations while protecting player experience, live-service safety, build stability, and content integrity.

## When to activate
Activate this skill for requests involving: **disconnect, reconnect, rejoin, timeout, session recovery, server crash, or host migration**.

Typical user prompts:
- Review the current implementation related to handling disconnects, reconnect, rejoin, timeout, host migration where applicable, server crash recovery, save handoff, and match integrity and produce a risk-ranked findings list.
- Modify the implementation for handling disconnects, reconnect, rejoin, timeout, host migration where applicable, server crash recovery, save handoff, and match integrity with a narrow patch and explain validation steps.
- Plan an upgrade or modernization path for handling disconnects, reconnect, rejoin, timeout, host migration where applicable, server crash recovery, save handoff, and match integrity that preserves live-game compatibility.

## Non-negotiable constraints
- Design every online flow around imperfect networks.
...
- Trace how handling disconnects, reconnect, rejoin, timeout, host migration where applicable, server crash recovery, save handoff, and match integrity enters the runtime: authoring data, code path, asset dependency, build/cook step, runtime initialization, telemetry, and failure reporting.
...
- Make changes around handling disconnects, reconnect, rejoin, timeout, host migration where applicable, server crash recovery, save handoff, and match integrity in the smallest coherent slice: one behavior, one migration, one rendering budget, one endpoint, or one asset pipeline rule at a time.

## Upgrade and modernization protocol
- Inventory current versions, owners, data formats, scripts, configs, runtime assumptions, and tests connected to handling disconnects, reconnect, rejoin, timeout, host migration where applicable, server crash recovery, save handoff, and match integrity.
...
- Representative assets/scenes/maps/services that exercise handling disconnects, reconnect, rejoin, timeout, host migration where applicable, server crash recovery, save handoff, and match integrity, including worst-case content and platform-specific variants.
