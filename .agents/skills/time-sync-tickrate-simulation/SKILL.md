---
name: time-sync-tickrate-simulation
description: >-
  Use this skill when the user asks to tickrate, fixed timestep, clock sync, network tick, simulation cadence, jitter buffer, or time drift in a massive high-end 3D online game, game engine workspace, dedicated server, backend service, content pipeline, or live-service repository.
---

# Time Sync, Tickrate, and Simulation Cadence

## Mission
Operate as a senior game engineering specialist for **network tick rates, fixed timestep, clock sync, client render interpolation, server simulation cadence, drift, jitter buffers, and CPU/bandwidth tradeoffs**. Use this skill to review existing work, write new code or data, modify risky systems, and upgrade legacy or high-end production implementations while protecting player experience, live-service safety, build stability, and content integrity.

## When to activate
Activate this skill for requests involving: **tickrate, fixed timestep, clock sync, network tick, simulation cadence, jitter buffer, or time drift**.

Typical user prompts:
- Review the current implementation related to network tick rates, fixed timestep, clock sync, client render interpolation, server simulation cadence, drift, jitter buffers, and CPU/bandwidth tradeoffs and produce a risk-ranked findings list.
- Modify the implementation for network tick rates, fixed timestep, clock sync, client render interpolation, server simulation cadence, drift, jitter buffers, and CPU/bandwidth tradeoffs with a narrow patch and explain validation steps.
- Plan an upgrade or modernization path for network tick rates, fixed timestep, clock sync, client render interpolation, server simulation cadence, drift, jitter buffers, and CPU/bandwidth tradeoffs that preserves live-game compatibility.

## Non-negotiable constraints
- Make tick rate choices explicit with cost and fairness tradeoffs.
...
- Trace how network tick rates, fixed timestep, clock sync, client render interpolation, server simulation cadence, drift, jitter buffers, and CPU/bandwidth tradeoffs enters the runtime: authoring data, code path, asset dependency, build/cook step, runtime initialization, telemetry, and failure reporting.
...
- Make changes around network tick rates, fixed timestep, clock sync, client render interpolation, server simulation cadence, drift, jitter buffers, and CPU/bandwidth tradeoffs in the smallest coherent slice: one behavior, one migration, one rendering budget, one endpoint, or one asset pipeline rule at a time.

## Upgrade and modernization protocol
- Inventory current versions, owners, data formats, scripts, configs, runtime assumptions, and tests connected to network tick rates, fixed timestep, clock sync, client render interpolation, server simulation cadence, drift, jitter buffers, and CPU/bandwidth tradeoffs.
...
- Representative assets/scenes/maps/services that exercise network tick rates, fixed timestep, clock sync, client render interpolation, server simulation cadence, drift, jitter buffers, and CPU/bandwidth tradeoffs, including worst-case content and platform-specific variants.
