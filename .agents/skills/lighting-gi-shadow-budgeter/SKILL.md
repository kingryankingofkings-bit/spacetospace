---
name: lighting-gi-shadow-budgeter
description: >-
  Use this skill when the user asks to lighting, global illumination, Lumen, SDFGI, shadows, lightmaps, probes, virtual shadow maps, ray tracing, or reflections in a massive high-end 3D online game, game engine workspace, dedicated server, backend service, content pipeline, or live-service repository.
---

# Lighting, GI, and Shadow Budgeter

## Mission
Operate as a senior game engineering specialist for **budgeting dynamic/static lighting, Lumen/SDFGI/lightmaps, probes, reflection captures, shadow cascades, virtual shadow maps, ray tracing, and baked workflows**. Use this skill to review existing work, write new code or data, modify risky systems, and upgrade legacy or high-end production implementations while protecting player experience, live-service safety, build stability, and content integrity.

## When to activate
Activate this skill for requests involving: **lighting, global illumination, Lumen, SDFGI, shadows, lightmaps, probes, virtual shadow maps, ray tracing, or reflections**.

Typical user prompts:
- Review the current implementation related to budgeting dynamic/static lighting, Lumen/SDFGI/lightmaps, probes, reflection captures, shadow cascades, virtual shadow maps, ray tracing, and baked workflows and produce a risk-ranked findings list.
- Modify the implementation for budgeting dynamic/static lighting, Lumen/SDFGI/lightmaps, probes, reflection captures, shadow cascades, virtual shadow maps, ray tracing, and baked workflows with a narrow patch and explain validation steps.
- Plan an upgrade or modernization path for budgeting dynamic/static lighting, Lumen/SDFGI/lightmaps, probes, reflection captures, shadow cascades, virtual shadow maps, ray tracing, and baked workflows that preserves live-game compatibility.

## Non-negotiable constraints
- Validate worst-case lights, multiplayer crowds, day/night cycles, and platform fallback paths.
...
- Trace how budgeting dynamic/static lighting, Lumen/SDFGI/lightmaps, probes, reflection captures, shadow cascades, virtual shadow maps, ray tracing, and baked workflows enters the runtime: authoring data, code path, asset dependency, build/cook step, runtime initialization, telemetry, and failure reporting.
...
- Make changes around budgeting dynamic/static lighting, Lumen/SDFGI/lightmaps, probes, reflection captures, shadow cascades, virtual shadow maps, ray tracing, and baked workflows in the smallest coherent slice: one behavior, one migration, one rendering budget, one endpoint, or one asset pipeline rule at a time.

## Upgrade and modernization protocol
- Inventory current versions, owners, data formats, scripts, configs, runtime assumptions, and tests connected to budgeting dynamic/static lighting, Lumen/SDFGI/lightmaps, probes, reflection captures, shadow cascades, virtual shadow maps, ray tracing, and baked workflows.
...
- Representative assets/scenes/maps/services that exercise budgeting dynamic/static lighting, Lumen/SDFGI/lightmaps, probes, reflection captures, shadow cascades, virtual shadow maps, ray tracing, and baked workflows, including worst-case content and platform-specific variants.
