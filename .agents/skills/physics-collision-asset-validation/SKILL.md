---
name: physics-collision-asset-validation
description: >-
  Use this skill when the user asks to collision, hitbox, physics asset, trigger, rigidbody, ragdoll bodies, collision channels, or query performance in a massive high-end 3D online game, game engine workspace, dedicated server, backend service, content pipeline, or live-service repository.
---

# Physics and Collision Asset Validation

## Mission
Operate as a senior game engineering specialist for **collision meshes, physics materials, destructible proxies, rigidbody setup, ragdoll bodies, hitboxes, triggers, query channels, and performance budgets**. Use this skill to review existing work, write new code or data, modify risky systems, and upgrade legacy or high-end production implementations while protecting player experience, live-service safety, build stability, and content integrity.

## When to activate
Activate this skill for requests involving: **collision, hitbox, physics asset, trigger, rigidbody, ragdoll bodies, collision channels, or query performance**.

Typical user prompts:
- Review the current implementation related to collision meshes, physics materials, destructible proxies, rigidbody setup, ragdoll bodies, hitboxes, triggers, query channels, and performance budgets and produce a risk-ranked findings list.
- Modify the implementation for collision meshes, physics materials, destructible proxies, rigidbody setup, ragdoll bodies, hitboxes, triggers, query channels, and performance budgets with a narrow patch and explain validation steps.
- Plan an upgrade or modernization path for collision meshes, physics materials, destructible proxies, rigidbody setup, ragdoll bodies, hitboxes, triggers, query channels, and performance budgets that preserves live-game compatibility.

## Non-negotiable constraints
- Separate gameplay collision from visual mesh complexity.
...
- Trace how collision meshes, physics materials, destructible proxies, rigidbody setup, ragdoll bodies, hitboxes, triggers, query channels, and performance budgets enters the runtime: authoring data, code path, asset dependency, build/cook step, runtime initialization, telemetry, and failure reporting.
...
- Make changes around collision meshes, physics materials, destructible proxies, rigidbody setup, ragdoll bodies, hitboxes, triggers, query channels, and performance budgets in the smallest coherent slice: one behavior, one migration, one rendering budget, one endpoint, or one asset pipeline rule at a time.

## Upgrade and modernization protocol
- Inventory current versions, owners, data formats, scripts, configs, runtime assumptions, and tests connected to collision meshes, physics materials, destructible proxies, rigidbody setup, ragdoll bodies, hitboxes, triggers, query channels, and performance budgets.
...
- Representative assets/scenes/maps/services that exercise collision meshes, physics materials, destructible proxies, rigidbody setup, ragdoll bodies, hitboxes, triggers, query channels, and performance budgets, including worst-case content and platform-specific variants.
