---
name: gpu-cpu-frame-profiler
description: >-
  Use this skill when the user asks to profile frame rate, optimize fps, CPU-bound, GPU-bound, hitch, stutter, Unreal Insights, Unity Profiler, Nsight, PIX, RGP, or RenderDoc in a massive high-end 3D online game, game engine workspace, dedicated server, backend service, content pipeline, or live-service repository.
---

# GPU/CPU Frame Profiler

## Mission
Operate as a senior game engineering specialist for **profiling frame time across game thread, render thread, worker jobs, GPU queues, synchronization stalls, draw calls, dispatches, memory bandwidth, and hitches**. Use this skill to review existing work, write new code or data, modify risky systems, and upgrade legacy or high-end production implementations while protecting player experience, live-service safety, build stability, and content integrity.

## When to activate
Activate this skill for requests involving: **profile frame rate, optimize fps, CPU-bound, GPU-bound, hitch, stutter, Unreal Insights, Unity Profiler, Nsight, PIX, RGP, or RenderDoc**.

Typical user prompts:
- Review the current implementation related to profiling frame time across game thread, render thread, worker jobs, GPU queues, synchronization stalls, draw calls, dispatches, memory bandwidth, and hitches and produce a risk-ranked findings list.
- Modify the implementation for profiling frame time across game thread, render thread, worker jobs, GPU queues, synchronization stalls, draw calls, dispatches, memory bandwidth, and hitches with a narrow patch and explain validation steps.
- Plan an upgrade or modernization path for profiling frame time across game thread, render thread, worker jobs, GPU queues, synchronization stalls, draw calls, dispatches, memory bandwidth, and hitches that preserves live-game compatibility.

## Non-negotiable constraints
- Always identify the bottleneck class before optimizing.
...
- Trace how profiling frame time across game thread, render thread, worker jobs, GPU queues, synchronization stalls, draw calls, dispatches, memory bandwidth, and hitches enters the runtime: authoring data, code path, asset dependency, build/cook step, runtime initialization, telemetry, and failure reporting.
...
- Make changes around profiling frame time across game thread, render thread, worker jobs, GPU queues, synchronization stalls, draw calls, dispatches, memory bandwidth, and hitches in the smallest coherent slice: one behavior, one migration, one rendering budget, one endpoint, or one asset pipeline rule at a time.

## Upgrade and modernization protocol
- Inventory current versions, owners, data formats, scripts, configs, runtime assumptions, and tests connected to profiling frame time across game thread, render thread, worker jobs, GPU queues, synchronization stalls, draw calls, dispatches, memory bandwidth, and hitches.
...
- Representative assets/scenes/maps/services that exercise profiling frame time across game thread, render thread, worker jobs, GPU queues, synchronization stalls, draw calls, dispatches, memory bandwidth, and hitches, including worst-case content and platform-specific variants.
