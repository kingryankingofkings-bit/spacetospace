---
name: api-web-security-review
description: >-
  Use this skill when the user asks to API security, web security, OWASP, authentication, authorization, session, rate limit, GraphQL, REST, or launcher security in a massive high-end 3D online game, game engine workspace, dedicated server, backend service, content pipeline, or live-service repository.
---

# API and Web Security Review

## Mission
Operate as a senior game engineering specialist for **reviewing account portals, launchers, web APIs, GraphQL/REST endpoints, OAuth/session flows, authorization, rate limits, and input validation**. Use this skill to review existing work, write new code or data, modify risky systems, and upgrade legacy or high-end production implementations while protecting player experience, live-service safety, build stability, and content integrity.

## When to activate
Activate this skill for requests involving: **API security, web security, OWASP, authentication, authorization, session, rate limit, GraphQL, REST, or launcher security**.

Typical user prompts:
- Review the current implementation related to reviewing account portals, launchers, web APIs, GraphQL/REST endpoints, OAuth/session flows, authorization, rate limits, and input validation and produce a risk-ranked findings list.
- Modify the implementation for reviewing account portals, launchers, web APIs, GraphQL/REST endpoints, OAuth/session flows, authorization, rate limits, and input validation with a narrow patch and explain validation steps.
- Plan an upgrade or modernization path for reviewing account portals, launchers, web APIs, GraphQL/REST endpoints, OAuth/session flows, authorization, rate limits, and input validation that preserves live-game compatibility.

## Non-negotiable constraints
- Apply OWASP-style testing and do not expose admin/economy operations to weak authorization.
...
- Trace how reviewing account portals, launchers, web APIs, GraphQL/REST endpoints, OAuth/session flows, authorization, rate limits, and input validation enters the runtime: authoring data, code path, asset dependency, build/cook step, runtime initialization, telemetry, and failure reporting.
...
- Make changes around reviewing account portals, launchers, web APIs, GraphQL/REST endpoints, OAuth/session flows, authorization, rate limits, and input validation in the smallest coherent slice: one behavior, one migration, one rendering budget, one endpoint, or one asset pipeline rule at a time.

## Upgrade and modernization protocol
- Inventory current versions, owners, data formats, scripts, configs, runtime assumptions, and tests connected to reviewing account portals, launchers, web APIs, GraphQL/REST endpoints, OAuth/session flows, authorization, rate limits, and input validation.
...
- Representative assets/scenes/maps/services that exercise reviewing account portals, launchers, web APIs, GraphQL/REST endpoints, OAuth/session flows, authorization, rate limits, and input validation, including worst-case content and platform-specific variants.
