---
name: online-error-ux-resilience
description: >-
  Use this skill when the user asks to online error, server unavailable, maintenance, retry, queue, matchmaking failed, auth failed, or degraded mode in a massive high-end 3D online game, game engine workspace, dedicated server, backend service, content pipeline, or live-service repository.
---

# Online Error UX and Resilience

## Mission
Operate as a senior game engineering specialist for **client-facing online errors, retries, degraded modes, maintenance screens, queue messaging, auth failures, matchmaking failures, and telemetry correlation**. Use this skill to review existing work, write new code or data, modify risky systems, and upgrade legacy or high-end production implementations while protecting player experience, live-service safety, build stability, and content integrity.

## When to activate
Activate this skill for requests involving: **online error, server unavailable, maintenance, retry, queue, matchmaking failed, auth failed, or degraded mode**.

Typical user prompts:
- Review the current implementation related to client-facing online errors, retries, degraded modes, maintenance screens, queue messaging, auth failures, matchmaking failures, and telemetry correlation and produce a risk-ranked findings list.
- Modify the implementation for client-facing online errors, retries, degraded modes, maintenance screens, queue messaging, auth failures, matchmaking failures, and telemetry correlation with a narrow patch and explain validation steps.
- Plan an upgrade or modernization path for client-facing online errors, retries, degraded modes, maintenance screens, queue messaging, auth failures, matchmaking failures, and telemetry correlation that preserves live-game compatibility.

## Non-negotiable constraints
- Make errors actionable for players and diagnosable for engineers.
...
- Trace how client-facing online errors, retries, degraded modes, maintenance screens, queue messaging, auth failures, matchmaking failures, and telemetry correlation enters the runtime: authoring data, code path, asset dependency, build/cook step, runtime initialization, telemetry, and failure reporting.
...
- Make changes around client-facing online errors, retries, degraded modes, maintenance screens, queue messaging, auth failures, matchmaking failures, and telemetry correlation in the smallest coherent slice: one behavior, one migration, one rendering budget, one endpoint, or one asset pipeline rule at a time.

## Upgrade and modernization protocol
- Inventory current versions, owners, data formats, scripts, configs, runtime assumptions, and tests connected to client-facing online errors, retries, degraded modes, maintenance screens, queue messaging, auth failures, matchmaking failures, and telemetry correlation.
...
- Representative assets/scenes/maps/services that exercise client-facing online errors, retries, degraded modes, maintenance screens, queue messaging, auth failures, matchmaking failures, and telemetry correlation, including worst-case content and platform-specific variants.
