---
name: player-identity-auth-sessions
description: >-
  Use this skill when the user asks to login, auth, session, JWT, OAuth, OIDC, account merge, device ID, or guest account in a massive high-end 3D online game, game engine workspace, dedicated server, backend service, content pipeline, or live-service repository.
---

# Player Identity, Auth, and Sessions

## Mission
Operate as a senior game engineering specialist for **login, identity linking, OAuth/OIDC, JWT/session handling, refresh tokens, device IDs, guest accounts, account merge, and platform auth**. Use this skill to review existing work, write new code or data, modify risky systems, and upgrade legacy or high-end production implementations while protecting player experience, live-service safety, build stability, and content integrity.

## When to activate
Activate this skill for requests involving: **login, auth, session, JWT, OAuth, OIDC, account merge, device ID, or guest account**.

Typical user prompts:
- Review the current implementation related to login, identity linking, OAuth/OIDC, JWT/session handling, refresh tokens, device IDs, guest accounts, account merge, and platform auth and produce a risk-ranked findings list.
- Modify the implementation for login, identity linking, OAuth/OIDC, JWT/session handling, refresh tokens, device IDs, guest accounts, account merge, and platform auth with a narrow patch and explain validation steps.
- Plan an upgrade or modernization path for login, identity linking, OAuth/OIDC, JWT/session handling, refresh tokens, device IDs, guest accounts, account merge, and platform auth that preserves live-game compatibility.

## Non-negotiable constraints
- Avoid token leakage and broken authorization; model account recovery and linking conflicts.
...
- Trace how login, identity linking, OAuth/OIDC, JWT/session handling, refresh tokens, device IDs, guest accounts, account merge, and platform auth enters the runtime: authoring data, code path, asset dependency, build/cook step, runtime initialization, telemetry, and failure reporting.
...
- Make changes around login, identity linking, OAuth/OIDC, JWT/session handling, refresh tokens, device IDs, guest accounts, account merge, and platform auth in the smallest coherent slice: one behavior, one migration, one rendering budget, one endpoint, or one asset pipeline rule at a time.

## Upgrade and modernization protocol
- Inventory current versions, owners, data formats, scripts, configs, runtime assumptions, and tests connected to login, identity linking, OAuth/OIDC, JWT/session handling, refresh tokens, device IDs, guest accounts, account merge, and platform auth.
...
- Representative assets/scenes/maps/services that exercise login, identity linking, OAuth/OIDC, JWT/session handling, refresh tokens, device IDs, guest accounts, account merge, and platform auth, including worst-case content and platform-specific variants.
