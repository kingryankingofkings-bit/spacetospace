---
name: open-world-streaming-world-partition
description: >-
  Use this skill when the user asks to open world, streaming, World Partition, additive scenes, chunks, cells, HLOD, fast travel, or large map in a massive high-end 3D online game, game engine workspace, dedicated server, backend service, content pipeline, or live-service repository.
---

# Open World Streaming and World Partition

## Mission
Operate as a senior game engineering specialist for **world streaming, Unreal World Partition, Unity scene streaming/Addressables, Godot large worlds, cells, HLODs, fast travel, server zones, and memory/IO budgets**. Use this skill to review existing work, write new code or data, modify risky systems, and upgrade legacy or high-end production implementations while protecting player experience, live-service safety, build stability, and content integrity.

## When to activate
Activate this skill for requests involving: **open world, streaming, World Partition, additive scenes, chunks, cells, HLOD, fast travel, or large map**.

Typical user prompts:
- Review the current implementation related to world streaming, Unreal World Partition, Unity scene streaming/Addressables, Godot large worlds, cells, HLODs, fast travel, server zones, and memory/IO budgets and produce a risk-ranked findings list.
- Modify the implementation for world streaming, Unreal World Partition, Unity scene streaming/Addressables, Godot large worlds, cells, HLODs, fast travel, server zones, and memory/IO budgets with a narrow patch and explain validation steps.
- Plan an upgrade or modernization path for world streaming, Unreal World Partition, Unity scene streaming/Addressables, Godot large worlds, cells, HLODs, fast travel, server zones, and memory/IO budgets that preserves live-game compatibility.

## Non-negotiable constraints
- Validate fast traversal and multiplayer/server zone behavior.
...
- Trace how world streaming, Unreal World Partition, Unity scene streaming/Addressables, Godot large worlds, cells, HLODs, fast travel, server zones, and memory/IO budgets enters the runtime: authoring data, code path, asset dependency, build/cook step, runtime initialization, telemetry, and failure reporting.
...
- Make changes around world streaming, Unreal World Partition, Unity scene streaming/Addressables, Godot large worlds, cells, HLODs, fast travel, server zones, and memory/IO budgets in the smallest coherent slice: one behavior, one migration, one rendering budget, one endpoint, or one asset pipeline rule at a time.

## Upgrade and modernization protocol
- Inventory current versions, owners, data formats, scripts, configs, runtime assumptions, and tests connected to world streaming, Unreal World Partition, Unity scene streaming/Addressables, Godot large worlds, cells, HLODs, fast travel, server zones, and memory/IO budgets.
...
- Representative assets/scenes/maps/services that exercise world streaming, Unreal World Partition, Unity scene streaming/Addressables, Godot large worlds, cells, HLODs, fast travel, server zones, and memory/IO budgets, including worst-case content and platform-specific variants.
