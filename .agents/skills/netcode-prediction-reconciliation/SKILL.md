---
name: netcode-prediction-reconciliation
description: >-
  Use this skill when the user asks to prediction, reconciliation, interpolation, rollback, input buffer, jitter, client-side prediction, or snap correction in a massive high-end 3D online game, game engine workspace, dedicated server, backend service, content pipeline, or live-service repository.
---

# Netcode Prediction and Reconciliation

## Mission
Operate as a senior game engineering specialist for **client prediction, server reconciliation, interpolation, extrapolation, rollback, input buffers, correction smoothing, prediction error telemetry, and local responsiveness**. Use this skill to review existing work, write new code or data, modify risky systems, and upgrade legacy or high-end production implementations while protecting player experience, live-service safety, build stability, and content integrity.

## When to activate
Activate this skill for requests involving: **prediction, reconciliation, interpolation, rollback, input buffer, jitter, client-side prediction, or snap correction**.

Typical user prompts:
- Review the current implementation related to client prediction, server reconciliation, interpolation, extrapolation, rollback, input buffers, correction smoothing, prediction error telemetry, and local responsiveness and produce a risk-ranked findings list.
- Modify the implementation for client prediction, server reconciliation, interpolation, extrapolation, rollback, input buffers, correction smoothing, prediction error telemetry, and local responsiveness with a narrow patch and explain validation steps.
- Plan an upgrade or modernization path for client prediction, server reconciliation, interpolation, extrapolation, rollback, input buffers, correction smoothing, prediction error telemetry, and local responsiveness that preserves live-game compatibility.

## Non-negotiable constraints
- Keep prediction cosmetic/temporary and server authority final.
...
- Trace how client prediction, server reconciliation, interpolation, extrapolation, rollback, input buffers, correction smoothing, prediction error telemetry, and local responsiveness enters the runtime: authoring data, code path, asset dependency, build/cook step, runtime initialization, telemetry, and failure reporting.
...
- Make changes around client prediction, server reconciliation, interpolation, extrapolation, rollback, input buffers, correction smoothing, prediction error telemetry, and local responsiveness in the smallest coherent slice: one behavior, one migration, one rendering budget, one endpoint, or one asset pipeline rule at a time.

## Upgrade and modernization protocol
- Inventory current versions, owners, data formats, scripts, configs, runtime assumptions, and tests connected to client prediction, server reconciliation, interpolation, extrapolation, rollback, input buffers, correction smoothing, prediction error telemetry, and local responsiveness.
...
- Representative assets/scenes/maps/services that exercise client prediction, server reconciliation, interpolation, extrapolation, rollback, input buffers, correction smoothing, prediction error telemetry, and local responsiveness, including worst-case content and platform-specific variants.
