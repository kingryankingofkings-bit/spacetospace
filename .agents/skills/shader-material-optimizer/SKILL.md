---
name: shader-material-optimizer
description: >-
  Use this skill when the user asks to shader, material, HLSL, GLSL, ShaderGraph, material graph, variant, keyword, compile stall, or GPU hotspot in a massive high-end 3D online game, game engine workspace, dedicated server, backend service, content pipeline, or live-service repository.
---

# Shader and Material Optimizer

## Mission
Operate as a senior game engineering specialist for **optimizing HLSL/GLSL/ShaderGraph/material graphs, variants, permutations, branching, texture fetches, precision, keyword explosion, and shader compilation stalls**. Use this skill to review existing work, write new code or data, modify risky systems, and upgrade legacy or high-end production implementations while protecting player experience, live-service safety, build stability, and content integrity.

## When to activate
Activate this skill for requests involving: **shader, material, HLSL, GLSL, ShaderGraph, material graph, variant, keyword, compile stall, or GPU hotspot**.

Typical user prompts:
- Review the current implementation related to optimizing HLSL/GLSL/ShaderGraph/material graphs, variants, permutations, branching, texture fetches, precision, keyword explosion, and shader compilation stalls and produce a risk-ranked findings list.
- Modify the implementation for optimizing HLSL/GLSL/ShaderGraph/material graphs, variants, permutations, branching, texture fetches, precision, keyword explosion, and shader compilation stalls with a narrow patch and explain validation steps.
- Plan an upgrade or modernization path for optimizing HLSL/GLSL/ShaderGraph/material graphs, variants, permutations, branching, texture fetches, precision, keyword explosion, and shader compilation stalls that preserves live-game compatibility.

## Non-negotiable constraints
- Preserve art direction and verify variant stripping does not break platforms/content.
...
- Trace how optimizing HLSL/GLSL/ShaderGraph/material graphs, variants, permutations, branching, texture fetches, precision, keyword explosion, and shader compilation stalls enters the runtime: authoring data, code path, asset dependency, build/cook step, runtime initialization, telemetry, and failure reporting.
...
- Make changes around optimizing HLSL/GLSL/ShaderGraph/material graphs, variants, permutations, branching, texture fetches, precision, keyword explosion, and shader compilation stalls in the smallest coherent slice: one behavior, one migration, one rendering budget, one endpoint, or one asset pipeline rule at a time.

## Upgrade and modernization protocol
- Inventory current versions, owners, data formats, scripts, configs, runtime assumptions, and tests connected to optimizing HLSL/GLSL/ShaderGraph/material graphs, variants, permutations, branching, texture fetches, precision, keyword explosion, and shader compilation stalls.
...
- Representative assets/scenes/maps/services that exercise optimizing HLSL/GLSL/ShaderGraph/material graphs, variants, permutations, branching, texture fetches, precision, keyword explosion, and shader compilation stalls, including worst-case content and platform-specific variants.
