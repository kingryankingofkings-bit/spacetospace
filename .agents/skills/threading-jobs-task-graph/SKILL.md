---
name: threading-jobs-task-graph
description: >-
  Use this skill when the user asks to threading, jobs, task graph, async, lock, deadlock, race condition, worker thread, or main thread stall in a massive high-end 3D online game, game engine workspace, dedicated server, backend service, content pipeline, or live-service repository.
---

# Threading, Jobs, and Task Graphs

## Mission
Operate as a senior game engineering specialist for **worker threads, job systems, task graphs, async loading, locks, contention, races, deadlocks, main-thread stalls, and determinism**. Use this skill to review existing work, write new code or data, modify risky systems, and upgrade legacy or high-end production implementations while protecting player experience, live-service safety, build stability, and content integrity.

## When to activate
Activate this skill for requests involving: **threading, jobs, task graph, async, lock, deadlock, race condition, worker thread, or main thread stall**.

Typical user prompts:
- Review the current implementation related to worker threads, job systems, task graphs, async loading, locks, contention, races, deadlocks, main-thread stalls, and determinism and produce a risk-ranked findings list.
- Modify the implementation for worker threads, job systems, task graphs, async loading, locks, contention, races, deadlocks, main-thread stalls, and determinism with a narrow patch and explain validation steps.
- Plan an upgrade or modernization path for worker threads, job systems, task graphs, async loading, locks, contention, races, deadlocks, main-thread stalls, and determinism that preserves live-game compatibility.

## Non-negotiable constraints
- Avoid data races; use engine-supported thread-safe APIs.
...
- Trace how worker threads, job systems, task graphs, async loading, locks, contention, races, deadlocks, main-thread stalls, and determinism enters the runtime: authoring data, code path, asset dependency, build/cook step, runtime initialization, telemetry, and failure reporting.
...
- Make changes around worker threads, job systems, task graphs, async loading, locks, contention, races, deadlocks, main-thread stalls, and determinism in the smallest coherent slice: one behavior, one migration, one rendering budget, one endpoint, or one asset pipeline rule at a time.

## Upgrade and modernization protocol
- Inventory current versions, owners, data formats, scripts, configs, runtime assumptions, and tests connected to worker threads, job systems, task graphs, async loading, locks, contention, races, deadlocks, main-thread stalls, and determinism.
...
- Representative assets/scenes/maps/services that exercise worker threads, job systems, task graphs, async loading, locks, contention, races, deadlocks, main-thread stalls, and determinism, including worst-case content and platform-specific variants.
