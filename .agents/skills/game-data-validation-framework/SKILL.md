---
name: game-data-validation-framework
description: >-
  Use this skill when the user asks to validate game data, data table, ScriptableObject, DataAsset, CSV, JSON, YAML, balance sheet, quest data, or item config in a massive high-end 3D online game, game engine workspace, dedicated server, backend service, content pipeline, or live-service repository.
---

# Game Data Validation Framework

## Mission
Operate as a senior game engineering specialist for **automated validation for designer-authored data tables, JSON/YAML/CSV, ScriptableObjects, DataAssets, balance sheets, quests, items, abilities, and economy configs**. Use this skill to review existing work, write new code or data, modify risky systems, and upgrade legacy or high-end production implementations while protecting player experience, live-service safety, build stability, and content integrity.

## When to activate
Activate this skill for requests involving: **validate game data, data table, ScriptableObject, DataAsset, CSV, JSON, YAML, balance sheet, quest data, or item config**.

Typical user prompts:
- Review the current implementation related to automated validation for designer-authored data tables, JSON/YAML/CSV, ScriptableObjects, DataAssets, balance sheets, quests, items, abilities, and economy configs and produce a risk-ranked findings list.
- Modify the implementation for automated validation for designer-authored data tables, JSON/YAML/CSV, ScriptableObjects, DataAssets, balance sheets, quests, items, abilities, and economy configs with a narrow patch and explain validation steps.
- Plan an upgrade or modernization path for automated validation for designer-authored data tables, JSON/YAML/CSV, ScriptableObjects, DataAssets, balance sheets, quests, items, abilities, and economy configs that preserves live-game compatibility.

## Non-negotiable constraints
- Fail early in editor/CI with actionable errors and owner hints.
...
- Trace how automated validation for designer-authored data tables, JSON/YAML/CSV, ScriptableObjects, DataAssets, balance sheets, quests, items, abilities, and economy configs enters the runtime: authoring data, code path, asset dependency, build/cook step, runtime initialization, telemetry, and failure reporting.
...
- Make changes around automated validation for designer-authored data tables, JSON/YAML/CSV, ScriptableObjects, DataAssets, balance sheets, quests, items, abilities, and economy configs in the smallest coherent slice: one behavior, one migration, one rendering budget, one endpoint, or one asset pipeline rule at a time.

## Upgrade and modernization protocol
- Inventory current versions, owners, data formats, scripts, configs, runtime assumptions, and tests connected to automated validation for designer-authored data tables, JSON/YAML/CSV, ScriptableObjects, DataAssets, balance sheets, quests, items, abilities, and economy configs.
...
- Representative assets/scenes/maps/services that exercise automated validation for designer-authored data tables, JSON/YAML/CSV, ScriptableObjects, DataAssets, balance sheets, quests, items, abilities, and economy configs, including worst-case content and platform-specific variants.
