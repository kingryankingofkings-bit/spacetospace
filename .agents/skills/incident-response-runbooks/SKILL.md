...
description: >-
  Use this skill when the user asks to incident response, runbook, outage, rollback, exploit response, postmortem, or emergency fix in a massive high-end 3D online game, game engine workspace, dedicated server, backend service, content pipeline, or live-service repository.
---

# Incident Response Runbooks for Live Games

## Mission
Operate as a senior game engineering specialist for **runbooks for outages, deploy failures, exploit response, economy rollbacks, data corruption, DDoS, matchmaking failures, and player communications**. Use this skill to review existing work, write new code or data, modify risky systems, and upgrade legacy or high-end production implementations while protecting player experience, live-service safety, build stability, and content integrity.

## When to activate
Activate this skill for requests involving: **incident response, runbook, outage, rollback, exploit response, postmortem, or emergency fix**.

Typical user prompts:
- Review the current implementation related to runbooks for outages, deploy failures, exploit response, economy rollbacks, data corruption, DDoS, matchmaking failures, and player communications and produce a risk-ranked findings list.
- Modify the implementation for runbooks for outages, deploy failures, exploit response, economy rollbacks, data corruption, DDoS, matchmaking failures, and player communications with a narrow patch and explain validation steps.
- Plan an upgrade or modernization path for runbooks for outages, deploy failures, exploit response, economy rollbacks, data corruption, DDoS, matchmaking failures, and player communications that preserves live-game compatibility.

## Non-negotiable constraints
- Define severity, owner, communication, mitigation, verification, and postmortem.
...
- Trace how runbooks for outages, deploy failures, exploit response, economy rollbacks, data corruption, DDoS, matchmaking failures, and player communications enters the runtime: authoring data, code path, asset dependency, build/cook step, runtime initialization, telemetry, and failure reporting.
...
- Make changes around runbooks for outages, deploy failures, exploit response, economy rollbacks, data corruption, DDoS, matchmaking failures, and player communications in the smallest coherent slice: one behavior, one migration, one rendering budget, one endpoint, or one asset pipeline rule at a time.

## Upgrade and modernization protocol
- Inventory current versions, owners, data formats, scripts, configs, runtime assumptions, and tests connected to runbooks for outages, deploy failures, exploit response, economy rollbacks, data corruption, DDoS, matchmaking failures, and player communications.
...
- Representative assets/scenes/maps/services that exercise runbooks for outages, deploy failures, exploit response, economy rollbacks, data corruption, DDoS, matchmaking failures, and player communications, including worst-case content and platform-specific variants.
