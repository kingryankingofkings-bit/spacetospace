---
name: qa-repro-bug-triage
description: >-
  Use this skill when the user asks to bug triage, repro steps, QA, severity, regression, test case, or reproduce bug in a massive high-end 3D online game, game engine workspace, dedicated server, backend service, content pipeline, or live-service repository.
---

# QA Reproduction and Bug Triage

## Mission
Operate as a senior game engineering specialist for **turning bug reports into minimal repros, severity, affected platforms, logs, save files, videos, suspected systems, and verification plans**. Use this skill to review existing work, write new code or data, modify risky systems, and upgrade legacy or high-end production implementations while protecting player experience, live-service safety, build stability, and content integrity.

## When to activate
Activate this skill for requests involving: **bug triage, repro steps, QA, severity, regression, test case, or reproduce bug**.

Typical user prompts:
- Review the current implementation related to turning bug reports into minimal repros, severity, affected platforms, logs, save files, videos, suspected systems, and verification plans and produce a risk-ranked findings list.
- Modify the implementation for turning bug reports into minimal repros, severity, affected platforms, logs, save files, videos, suspected systems, and verification plans with a narrow patch and explain validation steps.
- Plan an upgrade or modernization path for turning bug reports into minimal repros, severity, affected platforms, logs, save files, videos, suspected systems, and verification plans that preserves live-game compatibility.

## Non-negotiable constraints
- Distinguish reproduction facts from guesses and attach evidence.
...
- Trace how turning bug reports into minimal repros, severity, affected platforms, logs, save files, videos, suspected systems, and verification plans enters the runtime: authoring data, code path, asset dependency, build/cook step, runtime initialization, telemetry, and failure reporting.
...
- Make changes around turning bug reports into minimal repros, severity, affected platforms, logs, save files, videos, suspected systems, and verification plans in the smallest coherent slice: one behavior, one migration, one rendering budget, one endpoint, or one asset pipeline rule at a time.

## Upgrade and modernization protocol
- Inventory current versions, owners, data formats, scripts, configs, runtime assumptions, and tests connected to turning bug reports into minimal repros, severity, affected platforms, logs, save files, videos, suspected systems, and verification plans.
...
- Representative assets/scenes/maps/services that exercise turning bug reports into minimal repros, severity, affected platforms, logs, save files, videos, suspected systems, and verification plans, including worst-case content and platform-specific variants.
