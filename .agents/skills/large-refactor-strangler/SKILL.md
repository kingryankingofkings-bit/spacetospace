---
name: large-refactor-strangler
description: >-
  Use this skill when the user asks to refactor, rewrite, modernize, replace, decouple, modularize, move systems, or clean architecture in a large game in a massive high-end 3D online game, game engine workspace, dedicated server, backend service, content pipeline, or live-service repository.
---

# Large Refactor Strangler Pattern

## Mission
Operate as a senior game engineering specialist for **incrementally replacing fragile systems in a massive game without big-bang rewrites, regressions, or broken content references**. Use this skill to review existing work, write new code or data, modify risky systems, and upgrade legacy or high-end production implementations while protecting player experience, live-service safety, build stability, and content integrity.

## When to activate
Activate this skill for requests involving: **refactor, rewrite, modernize, replace, decouple, modularize, move systems, or clean architecture in a large game**.

Typical user prompts:
- Review the current implementation related to incrementally replacing fragile systems in a massive game without big-bang rewrites, regressions, or broken content references and produce a risk-ranked findings list.
- Modify the implementation for incrementally replacing fragile systems in a massive game without big-bang rewrites, regressions, or broken content references with a narrow patch and explain validation steps.
- Plan an upgrade or modernization path for incrementally replacing fragile systems in a massive game without big-bang rewrites, regressions, or broken content references that preserves live-game compatibility.

## Non-negotiable constraints
- Prefer adapters, compatibility layers, dual-run validation, feature flags, and small vertical slices.
...
- Trace how incrementally replacing fragile systems in a massive game without big-bang rewrites, regressions, or broken content references enters the runtime: authoring data, code path, asset dependency, build/cook step, runtime initialization, telemetry, and failure reporting.
...
- Make changes around incrementally replacing fragile systems in a massive game without big-bang rewrites, regressions, or broken content references in the smallest coherent slice: one behavior, one migration, one rendering budget, one endpoint, or one asset pipeline rule at a time.

## Upgrade and modernization protocol
- Inventory current versions, owners, data formats, scripts, configs, runtime assumptions, and tests connected to incrementally replacing fragile systems in a massive game without big-bang rewrites, regressions, or broken content references.
...
- Representative assets/scenes/maps/services that exercise incrementally replacing fragile systems in a massive game without big-bang rewrites, regressions, or broken content references, including worst-case content and platform-specific variants.
