---
name: sharding-zones-instancing
description: >-
  Use this skill when the user asks to sharding, zones, instance server, MMO, world server, server handoff, phasing, or megaserver in a massive high-end 3D online game, game engine workspace, dedicated server, backend service, content pipeline, or live-service repository.
---

# Sharding, Zones, and Instancing

## Mission
Operate as a senior game engineering specialist for **MMO/open-world region sharding, zone servers, instance servers, handoff, phasing, party placement, interest areas, and persistence coordination**. Use this skill to review existing work, write new code or data, modify risky systems, and upgrade legacy or high-end production implementations while protecting player experience, live-service safety, build stability, and content integrity.

## When to activate
Activate this skill for requests involving: **sharding, zones, instance server, MMO, world server, server handoff, phasing, or megaserver**.

Typical user prompts:
- Review the current implementation related to MMO/open-world region sharding, zone servers, instance servers, handoff, phasing, party placement, interest areas, and persistence coordination and produce a risk-ranked findings list.
- Modify the implementation for MMO/open-world region sharding, zone servers, instance servers, handoff, phasing, party placement, interest areas, and persistence coordination with a narrow patch and explain validation steps.
- Plan an upgrade or modernization path for MMO/open-world region sharding, zone servers, instance servers, handoff, phasing, party placement, interest areas, and persistence coordination that preserves live-game compatibility.

## Non-negotiable constraints
- Validate transitions, party cohesion, persistence, and failure isolation.
...
- Trace how MMO/open-world region sharding, zone servers, instance servers, handoff, phasing, party placement, interest areas, and persistence coordination enters the runtime: authoring data, code path, asset dependency, build/cook step, runtime initialization, telemetry, and failure reporting.
...
- Make changes around MMO/open-world region sharding, zone servers, instance servers, handoff, phasing, party placement, interest areas, and persistence coordination in the smallest coherent slice: one behavior, one migration, one rendering budget, one endpoint, or one asset pipeline rule at a time.

## Upgrade and modernization protocol
- Inventory current versions, owners, data formats, scripts, configs, runtime assumptions, and tests connected to MMO/open-world region sharding, zone servers, instance servers, handoff, phasing, party placement, interest areas, and persistence coordination.
...
- Representative assets/scenes/maps/services that exercise MMO/open-world region sharding, zone servers, instance servers, handoff, phasing, party placement, interest areas, and persistence coordination, including worst-case content and platform-specific variants.
