---
name: graphics-debugging-frame-capture
description: >-
  Use this skill when the user asks to frame capture, RenderDoc, Nsight, PIX, Radeon GPU Profiler, GPU debugging, shader debugging, frame debugger, or graphics bug in a massive high-end 3D online game, game engine workspace, dedicated server, backend service, content pipeline, or live-service repository.
---

# Graphics Debugging and Frame Capture

## Mission
Operate as a senior game engineering specialist for **using RenderDoc, Nsight Graphics, PIX, RGP, engine frame debuggers, GPU captures, shader inspection, render pass validation, and API correctness**. Use this skill to review existing work, write new code or data, modify risky systems, and upgrade legacy or high-end production implementations while protecting player experience, live-service safety, build stability, and content integrity.

## When to activate
Activate this skill for requests involving: **frame capture, RenderDoc, Nsight, PIX, Radeon GPU Profiler, GPU debugging, shader debugging, frame debugger, or graphics bug**.

Typical user prompts:
- Review the current implementation related to using RenderDoc, Nsight Graphics, PIX, RGP, engine frame debuggers, GPU captures, shader inspection, render pass validation, and API correctness and produce a risk-ranked findings list.
- Modify the implementation for using RenderDoc, Nsight Graphics, PIX, RGP, engine frame debuggers, GPU captures, shader inspection, render pass validation, and API correctness with a narrow patch and explain validation steps.
- Plan an upgrade or modernization path for using RenderDoc, Nsight Graphics, PIX, RGP, engine frame debuggers, GPU captures, shader inspection, render pass validation, and API correctness that preserves live-game compatibility.

## Non-negotiable constraints
- Record capture environment and avoid drawing conclusions from one frame without context.
...
- Trace how using RenderDoc, Nsight Graphics, PIX, RGP, engine frame debuggers, GPU captures, shader inspection, render pass validation, and API correctness enters the runtime: authoring data, code path, asset dependency, build/cook step, runtime initialization, telemetry, and failure reporting.
...
- Make changes around using RenderDoc, Nsight Graphics, PIX, RGP, engine frame debuggers, GPU captures, shader inspection, render pass validation, and API correctness in the smallest coherent slice: one behavior, one migration, one rendering budget, one endpoint, or one asset pipeline rule at a time.

## Upgrade and modernization protocol
- Inventory current versions, owners, data formats, scripts, configs, runtime assumptions, and tests connected to using RenderDoc, Nsight Graphics, PIX, RGP, engine frame debuggers, GPU captures, shader inspection, render pass validation, and API correctness.
...
- Representative assets/scenes/maps/services that exercise using RenderDoc, Nsight Graphics, PIX, RGP, engine frame debuggers, GPU captures, shader inspection, render pass validation, and API correctness, including worst-case content and platform-specific variants.
