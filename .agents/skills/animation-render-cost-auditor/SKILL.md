---
name: animation-render-cost-auditor
description: >-
  Use this skill when the user asks to skinned mesh, morph target, cloth, hair, fur, animation LOD, bone count, crowd rendering, or GPU skinning in a massive high-end 3D online game, game engine workspace, dedicated server, backend service, content pipeline, or live-service repository.
---

# Animation Render Cost Auditor

## Mission
Operate as a senior game engineering specialist for **render-side cost of skinned meshes, morph targets, cloth, hair/fur, animation LODs, crowd rendering, bone counts, GPU skinning, and update rate optimization**. Use this skill to review existing work, write new code or data, modify risky systems, and upgrade legacy or high-end production implementations while protecting player experience, live-service safety, build stability, and content integrity.

## When to activate
Activate this skill for requests involving: **skinned mesh, morph target, cloth, hair, fur, animation LOD, bone count, crowd rendering, or GPU skinning**.

Typical user prompts:
- Review the current implementation related to render-side cost of skinned meshes, morph targets, cloth, hair/fur, animation LODs, crowd rendering, bone counts, GPU skinning, and update rate optimization and produce a risk-ranked findings list.
- Modify the implementation for render-side cost of skinned meshes, morph targets, cloth, hair/fur, animation LODs, crowd rendering, bone counts, GPU skinning, and update rate optimization with a narrow patch and explain validation steps.
- Plan an upgrade or modernization path for render-side cost of skinned meshes, morph targets, cloth, hair/fur, animation LODs, crowd rendering, bone counts, GPU skinning, and update rate optimization that preserves live-game compatibility.

## Non-negotiable constraints
- Coordinate with gameplay animation skill; do not optimize away hitbox/visibility correctness.
...
- Trace how render-side cost of skinned meshes, morph targets, cloth, hair/fur, animation LODs, crowd rendering, bone counts, GPU skinning, and update rate optimization enters the runtime: authoring data, code path, asset dependency, build/cook step, runtime initialization, telemetry, and failure reporting.
...
- Make changes around render-side cost of skinned meshes, morph targets, cloth, hair/fur, animation LODs, crowd rendering, bone counts, GPU skinning, and update rate optimization in the smallest coherent slice: one behavior, one migration, one rendering budget, one endpoint, or one asset pipeline rule at a time.

## Upgrade and modernization protocol
- Inventory current versions, owners, data formats, scripts, configs, runtime assumptions, and tests connected to render-side cost of skinned meshes, morph targets, cloth, hair/fur, animation LODs, crowd rendering, bone counts, GPU skinning, and update rate optimization.
...
- Representative assets/scenes/maps/services that exercise render-side cost of skinned meshes, morph targets, cloth, hair/fur, animation LODs, crowd rendering, bone counts, GPU skinning, and update rate optimization, including worst-case content and platform-specific variants.
