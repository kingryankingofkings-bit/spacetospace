---
name: content-cooking-packaging-patching
description: >-
  Use this skill when the user asks to cook content, package build, patch, manifest, compression, chunk assignment, upload to CDN, or release artifact in a massive high-end 3D online game, game engine workspace, dedicated server, backend service, content pipeline, or live-service repository.
---

# Content Cooking, Packaging, and Patching

## Mission
Operate as a senior game engineering specialist for **cooking/building game content, packaging, compression, chunk assignment, patch generation, CDN upload, manifests, and rollback-friendly release artifacts**. Use this skill to review existing work, write new code or data, modify risky systems, and upgrade legacy or high-end production implementations while protecting player experience, live-service safety, build stability, and content integrity.

## When to activate
Activate this skill for requests involving: **cook content, package build, patch, manifest, compression, chunk assignment, upload to CDN, or release artifact**.

Typical user prompts:
- Review the current implementation related to cooking/building game content, packaging, compression, chunk assignment, patch generation, CDN upload, manifests, and rollback-friendly release artifacts and produce a risk-ranked findings list.
- Modify the implementation for cooking/building game content, packaging, compression, chunk assignment, patch generation, CDN upload, manifests, and rollback-friendly release artifacts with a narrow patch and explain validation steps.
- Plan an upgrade or modernization path for cooking/building game content, packaging, compression, chunk assignment, patch generation, CDN upload, manifests, and rollback-friendly release artifacts that preserves live-game compatibility.

## Non-negotiable constraints
- Protect deterministic outputs and patch-size budgets.
...
- Trace how cooking/building game content, packaging, compression, chunk assignment, patch generation, CDN upload, manifests, and rollback-friendly release artifacts enters the runtime: authoring data, code path, asset dependency, build/cook step, runtime initialization, telemetry, and failure reporting.
...
- Make changes around cooking/building game content, packaging, compression, chunk assignment, patch generation, CDN upload, manifests, and rollback-friendly release artifacts in the smallest coherent slice: one behavior, one migration, one rendering budget, one endpoint, or one asset pipeline rule at a time.

## Upgrade and modernization protocol
- Inventory current versions, owners, data formats, scripts, configs, runtime assumptions, and tests connected to cooking/building game content, packaging, compression, chunk assignment, patch generation, CDN upload, manifests, and rollback-friendly release artifacts.
...
- Representative assets/scenes/maps/services that exercise cooking/building game content, packaging, compression, chunk assignment, patch generation, CDN upload, manifests, and rollback-friendly release artifacts, including worst-case content and platform-specific variants.
