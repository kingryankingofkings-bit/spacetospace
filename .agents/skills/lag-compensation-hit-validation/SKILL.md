---
name: lag-compensation-hit-validation
description: >-
  Use this skill when the user asks to lag compensation, hit reg, server rewind, hit validation, projectile, melee, peeker advantage, or clock sync in a massive high-end 3D online game, game engine workspace, dedicated server, backend service, content pipeline, or live-service repository.
---

# Lag Compensation and Hit Validation

## Mission
Operate as a senior game engineering specialist for **server-side rewind, hit registration, projectile validation, melee validation, clock sync, peeker advantage, fairness, replay evidence, and anti-cheat constraints**. Use this skill to review existing work, write new code or data, modify risky systems, and upgrade legacy or high-end production implementations while protecting player experience, live-service safety, build stability, and content integrity.

## When to activate
Activate this skill for requests involving: **lag compensation, hit reg, server rewind, hit validation, projectile, melee, peeker advantage, or clock sync**.

Typical user prompts:
- Review the current implementation related to server-side rewind, hit registration, projectile validation, melee validation, clock sync, peeker advantage, fairness, replay evidence, and anti-cheat constraints and produce a risk-ranked findings list.
- Modify the implementation for server-side rewind, hit registration, projectile validation, melee validation, clock sync, peeker advantage, fairness, replay evidence, and anti-cheat constraints with a narrow patch and explain validation steps.
- Plan an upgrade or modernization path for server-side rewind, hit registration, projectile validation, melee validation, clock sync, peeker advantage, fairness, replay evidence, and anti-cheat constraints that preserves live-game compatibility.

## Non-negotiable constraints
- Balance fairness, exploit resistance, and player-perceived responsiveness.
...
## Master workflow
1. Classify each state as authoritative server state, predicted client state, cosmetic client state, replicated state, persistent account state, or analytics-only state.
1. Design around latency, packet loss, jitter, bandwidth, concurrency, cheating, disconnects, rejoin, server migration, and deploy rollback.
1. Use explicit protocol/versioning, input validation, bounds checks, rate limits, abuse controls, server-side replay/audit logs, and deterministic or reconciled state flows.
1. Test with simulated bad networks, bot swarms, region distance, server CPU saturation, serialization fuzzing, and malicious client attempts.

## Review protocol
- Audit RPC ownership, replication frequency, interest management, delta compression, prioritization, prediction/reconciliation, lag compensation, time sync, and server authority boundaries.
- Check hidden online dependencies: identity provider, friends/party, matchmaking, inventory/economy, entitlements, chat/voice, telemetry, anti-cheat, and moderation.
- Confirm LiveOps compatibility: rolling deploys, mixed client versions, feature flags, graceful degradation, queue/backoff behavior, and incident diagnostics.
- Trace how server-side rewind, hit registration, projectile validation, melee validation, clock sync, peeker advantage, fairness, replay evidence, and anti-cheat constraints enters the runtime: authoring data, code path, asset dependency, build/cook step, runtime initialization, telemetry, and failure reporting.
...
- Make changes around server-side rewind, hit registration, projectile validation, melee validation, clock sync, peeker advantage, fairness, replay evidence, and anti-cheat constraints in the smallest coherent slice: one behavior, one migration, one rendering budget, one endpoint, or one asset pipeline rule at a time.

## Upgrade and modernization protocol
- Inventory current versions, owners, data formats, scripts, configs, runtime assumptions, and tests connected to server-side rewind, hit registration, projectile validation, melee validation, clock sync, peeker advantage, fairness, replay evidence, and anti-cheat constraints.
...
- Representative assets/scenes/maps/services that exercise server-side rewind, hit registration, projectile validation, melee validation, clock sync, peeker advantage, fairness, replay evidence, and anti-cheat constraints, including worst-case content and platform-specific variants.
