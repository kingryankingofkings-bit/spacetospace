---
name: serialization-versioning-migrator
description: >-
  Use this skill when the user asks to serialization, save version, schema version, asset migration, GUID, prefab migration, blueprint migration, or backwards compatibility in a massive high-end 3D online game, game engine workspace, dedicated server, backend service, content pipeline, or live-service repository.
---

# Serialization Versioning Migrator

## Mission
Operate as a senior game engineering specialist for **versioning serialized assets, save games, network payloads, scriptable data, blueprints/prefabs/scenes, editor assets, and database-backed player state**. Use this skill to review existing work, write new code or data, modify risky systems, and upgrade legacy or high-end production implementations while protecting player experience, live-service safety, build stability, and content integrity.

## When to activate
Activate this skill for requests involving: **serialization, save version, schema version, asset migration, GUID, prefab migration, blueprint migration, or backwards compatibility**.

Typical user prompts:
- Review the current implementation related to versioning serialized assets, save games, network payloads, scriptable data, blueprints/prefabs/scenes, editor assets, and database-backed player state and produce a risk-ranked findings list.
- Modify the implementation for versioning serialized assets, save games, network payloads, scriptable data, blueprints/prefabs/scenes, editor assets, and database-backed player state with a narrow patch and explain validation steps.
- Plan an upgrade or modernization path for versioning serialized assets, save games, network payloads, scriptable data, blueprints/prefabs/scenes, editor assets, and database-backed player state that preserves live-game compatibility.

## Non-negotiable constraints
- Write idempotent migrations and keep rollback/compatibility paths.
...
- Trace how versioning serialized assets, save games, network payloads, scriptable data, blueprints/prefabs/scenes, editor assets, and database-backed player state enters the runtime: authoring data, code path, asset dependency, build/cook step, runtime initialization, telemetry, and failure reporting.
...
- Make changes around versioning serialized assets, save games, network payloads, scriptable data, blueprints/prefabs/scenes, editor assets, and database-backed player state in the smallest coherent slice: one behavior, one migration, one rendering budget, one endpoint, or one asset pipeline rule at a time.

## Upgrade and modernization protocol
- Inventory current versions, owners, data formats, scripts, configs, runtime assumptions, and tests connected to versioning serialized assets, save games, network payloads, scriptable data, blueprints/prefabs/scenes, editor assets, and database-backed player state.
...
- Representative assets/scenes/maps/services that exercise versioning serialized assets, save games, network payloads, scriptable data, blueprints/prefabs/scenes, editor assets, and database-backed player state, including worst-case content and platform-specific variants.
