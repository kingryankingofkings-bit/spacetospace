---
name: animation-locomotion-state-machines
description: >-
  Use this skill when the user asks to animation blueprint, locomotion, blend tree, motion matching, IK, root motion, animation state machine, or notifies in a massive high-end 3D online game, game engine workspace, dedicated server, backend service, content pipeline, or live-service repository.
---

# Animation, Locomotion, and State Machines

## Mission
Operate as a senior game engineering specialist for **locomotion graphs, blend trees, animation blueprints/controllers, root motion, IK, motion matching, state transitions, notifies/events, and network sync**. Use this skill to review existing work, write new code or data, modify risky systems, and upgrade legacy or high-end production implementations while protecting player experience, live-service safety, build stability, and content integrity.

## When to activate
Activate this skill for requests involving: **animation blueprint, locomotion, blend tree, motion matching, IK, root motion, animation state machine, or notifies**.

Typical user prompts:
- Review the current implementation related to locomotion graphs, blend trees, animation blueprints/controllers, root motion, IK, motion matching, state transitions, notifies/events, and network sync and produce a risk-ranked findings list.
- Modify the implementation for locomotion graphs, blend trees, animation blueprints/controllers, root motion, IK, motion matching, state transitions, notifies/events, and network sync with a narrow patch and explain validation steps.
- Plan an upgrade or modernization path for locomotion graphs, blend trees, animation blueprints/controllers, root motion, IK, motion matching, state transitions, notifies/events, and network sync that preserves live-game compatibility.

## Non-negotiable constraints
- Check sync with movement, combat, hitboxes, and prediction.
...
- Trace how locomotion graphs, blend trees, animation blueprints/controllers, root motion, IK, motion matching, state transitions, notifies/events, and network sync enters the runtime: authoring data, code path, asset dependency, build/cook step, runtime initialization, telemetry, and failure reporting.
...
- Make changes around locomotion graphs, blend trees, animation blueprints/controllers, root motion, IK, motion matching, state transitions, notifies/events, and network sync in the smallest coherent slice: one behavior, one migration, one rendering budget, one endpoint, or one asset pipeline rule at a time.

## Upgrade and modernization protocol
- Inventory current versions, owners, data formats, scripts, configs, runtime assumptions, and tests connected to locomotion graphs, blend trees, animation blueprints/controllers, root motion, IK, motion matching, state transitions, notifies/events, and network sync.
...
- Representative assets/scenes/maps/services that exercise locomotion graphs, blend trees, animation blueprints/controllers, root motion, IK, motion matching, state transitions, notifies/events, and network sync, including worst-case content and platform-specific variants.
