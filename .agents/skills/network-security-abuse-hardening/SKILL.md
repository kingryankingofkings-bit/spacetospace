---
name: network-security-abuse-hardening
description: >-
  Use this skill when the user asks to network security, DDoS, packet validation, rate limit, replay attack, UDP, QUIC, NAT traversal, or abusive client in a massive high-end 3D online game, game engine workspace, dedicated server, backend service, content pipeline, or live-service repository.
---

# Network Security and Abuse Hardening

## Mission
Operate as a senior game engineering specialist for **DDoS resilience, rate limiting, packet validation, replay protection, auth tokens, UDP/TCP/QUIC considerations, NAT traversal, and abusive clients**. Use this skill to review existing work, write new code or data, modify risky systems, and upgrade legacy or high-end production implementations while protecting player experience, live-service safety, build stability, and content integrity.

## When to activate
Activate this skill for requests involving: **network security, DDoS, packet validation, rate limit, replay attack, UDP, QUIC, NAT traversal, or abusive client**.

Typical user prompts:
- Review the current implementation related to DDoS resilience, rate limiting, packet validation, replay protection, auth tokens, UDP/TCP/QUIC considerations, NAT traversal, and abusive clients and produce a risk-ranked findings list.
- Modify the implementation for DDoS resilience, rate limiting, packet validation, replay protection, auth tokens, UDP/TCP/QUIC considerations, NAT traversal, and abusive clients with a narrow patch and explain validation steps.
- Plan an upgrade or modernization path for DDoS resilience, rate limiting, packet validation, replay protection, auth tokens, UDP/TCP/QUIC considerations, NAT traversal, and abusive clients that preserves live-game compatibility.

## Non-negotiable constraints
- Do not trust client packet timing, order, content, or identity without validation.
...
- Trace how DDoS resilience, rate limiting, packet validation, replay protection, auth tokens, UDP/TCP/QUIC considerations, NAT traversal, and abusive clients enters the runtime: authoring data, code path, asset dependency, build/cook step, runtime initialization, telemetry, and failure reporting.
...
- Make changes around DDoS resilience, rate limiting, packet validation, replay protection, auth tokens, UDP/TCP/QUIC considerations, NAT traversal, and abusive clients in the smallest coherent slice: one behavior, one migration, one rendering budget, one endpoint, or one asset pipeline rule at a time.

## Upgrade and modernization protocol
- Inventory current versions, owners, data formats, scripts, configs, runtime assumptions, and tests connected to DDoS resilience, rate limiting, packet validation, replay protection, auth tokens, UDP/TCP/QUIC considerations, NAT traversal, and abusive clients.
...
- Representative assets/scenes/maps/services that exercise DDoS resilience, rate limiting, packet validation, replay protection, auth tokens, UDP/TCP/QUIC considerations, NAT traversal, and abusive clients, including worst-case content and platform-specific variants.
