---
name: engine-fork-maintenance
description: >-
  Use this skill when the user asks to engine fork, custom engine patch, upstream merge, source build, Epic source, Unity source package, or engine patchset in a massive high-end 3D online game, game engine workspace, dedicated server, backend service, content pipeline, or live-service repository.
---

# Engine Fork Maintenance

## Mission
Operate as a senior game engineering specialist for **maintaining source-available engine forks, upstream syncs, local patches, conflict resolution, module boundaries, and rebase/merge strategy**. Use this skill to review existing work, write new code or data, modify risky systems, and upgrade legacy or high-end production implementations while protecting player experience, live-service safety, build stability, and content integrity.

## When to activate
Activate this skill for requests involving: **engine fork, custom engine patch, upstream merge, source build, Epic source, Unity source package, or engine patchset**.

Typical user prompts:
- Review the current implementation related to maintaining source-available engine forks, upstream syncs, local patches, conflict resolution, module boundaries, and rebase/merge strategy and produce a risk-ranked findings list.
- Modify the implementation for maintaining source-available engine forks, upstream syncs, local patches, conflict resolution, module boundaries, and rebase/merge strategy with a narrow patch and explain validation steps.
- Plan an upgrade or modernization path for maintaining source-available engine forks, upstream syncs, local patches, conflict resolution, module boundaries, and rebase/merge strategy that preserves live-game compatibility.

## Non-negotiable constraints
- Minimize fork delta, document patches, and keep testable upstream sync checkpoints.
...
- Trace how maintaining source-available engine forks, upstream syncs, local patches, conflict resolution, module boundaries, and rebase/merge strategy enters the runtime: authoring data, code path, asset dependency, build/cook step, runtime initialization, telemetry, and failure reporting.
...
- Make changes around maintaining source-available engine forks, upstream syncs, local patches, conflict resolution, module boundaries, and rebase/merge strategy in the smallest coherent slice: one behavior, one migration, one rendering budget, one endpoint, or one asset pipeline rule at a time.

## Upgrade and modernization protocol
- Inventory current versions, owners, data formats, scripts, configs, runtime assumptions, and tests connected to maintaining source-available engine forks, upstream syncs, local patches, conflict resolution, module boundaries, and rebase/merge strategy.
...
- Representative assets/scenes/maps/services that exercise maintaining source-available engine forks, upstream syncs, local patches, conflict resolution, module boundaries, and rebase/merge strategy, including worst-case content and platform-specific variants.
