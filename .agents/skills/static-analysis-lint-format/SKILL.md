...
description: >-
  Use this skill when the user asks to lint, formatter, static analysis, clang-tidy, Roslyn, ESLint, style, or code quality gate in a massive high-end 3D online game, game engine workspace, dedicated server, backend service, content pipeline, or live-service repository.
---

# Static Analysis, Lint, and Format Gate

## Mission
Operate as a senior game engineering specialist for **static analyzers, linters, formatters, codegen checks, style gates, clang-tidy, Roslyn analyzers, nullability, and dangerous pattern detection**. Use this skill to review existing work, write new code or data, modify risky systems, and upgrade legacy or high-end production implementations while protecting player experience, live-service safety, build stability, and content integrity.

## When to activate
Activate this skill for requests involving: **lint, formatter, static analysis, clang-tidy, Roslyn, ESLint, style, or code quality gate**.

Typical user prompts:
- Review the current implementation related to static analyzers, linters, formatters, codegen checks, style gates, clang-tidy, Roslyn analyzers, nullability, and dangerous pattern detection and produce a risk-ranked findings list.
- Modify the implementation for static analyzers, linters, formatters, codegen checks, style gates, clang-tidy, Roslyn analyzers, nullability, and dangerous pattern detection with a narrow patch and explain validation steps.
- Plan an upgrade or modernization path for static analyzers, linters, formatters, codegen checks, style gates, clang-tidy, Roslyn analyzers, nullability, and dangerous pattern detection that preserves live-game compatibility.

## Non-negotiable constraints
- Automate low-risk cleanup separately from behavior changes.
...
- Trace how static analyzers, linters, formatters, codegen checks, style gates, clang-tidy, Roslyn analyzers, nullability, and dangerous pattern detection enters the runtime: authoring data, code path, asset dependency, build/cook step, runtime initialization, telemetry, and failure reporting.
...
- Make changes around static analyzers, linters, formatters, codegen checks, style gates, clang-tidy, Roslyn analyzers, nullability, and dangerous pattern detection in the smallest coherent slice: one behavior, one migration, one rendering budget, one endpoint, or one asset pipeline rule at a time.

## Upgrade and modernization protocol
- Inventory current versions, owners, data formats, scripts, configs, runtime assumptions, and tests connected to static analyzers, linters, formatters, codegen checks, style gates, clang-tidy, Roslyn analyzers, nullability, and dangerous pattern detection.
...
- Representative assets/scenes/maps/services that exercise static analyzers, linters, formatters, codegen checks, style gates, clang-tidy, Roslyn analyzers, nullability, and dangerous pattern detection, including worst-case content and platform-specific variants.
