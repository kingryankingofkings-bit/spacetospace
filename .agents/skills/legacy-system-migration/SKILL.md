...
description: >-
  Use this skill when the user asks to legacy migration, port old system, replace old code, migrate data, or deprecate system in a massive high-end 3D online game, game engine workspace, dedicated server, backend service, content pipeline, or live-service repository.
---

# Legacy System Migration

## Mission
Operate as a senior game engineering specialist for **migrating old gameplay, assets, UI, services, save data, networking, build systems, or engine versions without breaking live players**. Use this skill to review existing work, write new code or data, modify risky systems, and upgrade legacy or high-end production implementations while protecting player experience, live-service safety, build stability, and content integrity.

## When to activate
Activate this skill for requests involving: **legacy migration, port old system, replace old code, migrate data, or deprecate system**.

Typical user prompts:
- Review the current implementation related to migrating old gameplay, assets, UI, services, save data, networking, build systems, or engine versions without breaking live players and produce a risk-ranked findings list.
- Modify the implementation for migrating old gameplay, assets, UI, services, save data, networking, build systems, or engine versions without breaking live players with a narrow patch and explain validation steps.
- Plan an upgrade or modernization path for migrating old gameplay, assets, UI, services, save data, networking, build systems, or engine versions without breaking live players that preserves live-game compatibility.

## Non-negotiable constraints
- Use parallel run, adapters, conversion scripts, data validation, and rollback.
...
- Trace how migrating old gameplay, assets, UI, services, save data, networking, build systems, or engine versions without breaking live players enters the runtime: authoring data, code path, asset dependency, build/cook step, runtime initialization, telemetry, and failure reporting.
...
- Make changes around migrating old gameplay, assets, UI, services, save data, networking, build systems, or engine versions without breaking live players in the smallest coherent slice: one behavior, one migration, one rendering budget, one endpoint, or one asset pipeline rule at a time.

## Upgrade and modernization protocol
- Inventory current versions, owners, data formats, scripts, configs, runtime assumptions, and tests connected to migrating old gameplay, assets, UI, services, save data, networking, build systems, or engine versions without breaking live players.
...
- Representative assets/scenes/maps/services that exercise migrating old gameplay, assets, UI, services, save data, networking, build systems, or engine versions without breaking live players, including worst-case content and platform-specific variants.
