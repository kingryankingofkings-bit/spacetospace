---
name: agentic-safety-guardrails
description: >-
  Use this skill when the user asks to delete, clean, reset, reinstall, migrate, refactor broadly, run scripts, change infrastructure, or execute commands in a game repository in a massive high-end 3D online game, game engine workspace, dedicated server, backend service, content pipeline, or live-service repository.
---

# Agentic Safety Guardrails for Antigravity Game Repos

## Mission
Operate as a senior game engineering specialist for **preventing catastrophic workspace damage, unsafe terminal commands, broad deletes, accidental asset loss, secret exposure, destructive migrations, and uncontrolled autonomous operations**. Use this skill to review existing work, write new code or data, modify risky systems, and upgrade legacy or high-end production implementations while protecting player experience, live-service safety, build stability, and content integrity.

## When to activate
Activate this skill for requests involving: **delete, clean, reset, reinstall, migrate, refactor broadly, run scripts, change infrastructure, or execute commands in a game repository**.

Typical user prompts:
- Review the current implementation related to preventing catastrophic workspace damage, unsafe terminal commands, broad deletes, accidental asset loss, secret exposure, destructive migrations, and uncontrolled autonomous operations and produce a risk-ranked findings list.
- Modify the implementation for preventing catastrophic workspace damage, unsafe terminal commands, broad deletes, accidental asset loss, secret exposure, destructive migrations, and uncontrolled autonomous operations with a narrow patch and explain validation steps.
- Plan an upgrade or modernization path for preventing catastrophic workspace damage, unsafe terminal commands, broad deletes, accidental asset loss, secret exposure, destructive migrations, and uncontrolled autonomous operations that preserves live-game compatibility.

## Non-negotiable constraints
- Require path verification, dry-run mindset, rollback, and human-readable diffs for destructive or irreversible work.
...
- Trace how preventing catastrophic workspace damage, unsafe terminal commands, broad deletes, accidental asset loss, secret exposure, destructive migrations, and uncontrolled autonomous operations enters the runtime: authoring data, code path, asset dependency, build/cook step, runtime initialization, telemetry, and failure reporting.
...
- Make changes around preventing catastrophic workspace damage, unsafe terminal commands, broad deletes, accidental asset loss, secret exposure, destructive migrations, and uncontrolled autonomous operations in the smallest coherent slice: one behavior, one migration, one rendering budget, one endpoint, or one asset pipeline rule at a time.

## Upgrade and modernization protocol
- Inventory current versions, owners, data formats, scripts, configs, runtime assumptions, and tests connected to preventing catastrophic workspace damage, unsafe terminal commands, broad deletes, accidental asset loss, secret exposure, destructive migrations, and uncontrolled autonomous operations.
...
- Representative assets/scenes/maps/services that exercise preventing catastrophic workspace damage, unsafe terminal commands, broad deletes, accidental asset loss, secret exposure, destructive migrations, and uncontrolled autonomous operations, including worst-case content and platform-specific variants.
