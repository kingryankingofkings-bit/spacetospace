---
name: crash-dump-symbolication
description: >-
  Use this skill when the user asks to crash, minidump, symbolication, stack trace, crash report, fatal error, or crash cluster in a massive high-end 3D online game, game engine workspace, dedicated server, backend service, content pipeline, or live-service repository.
---

# Crash Dump and Symbolication

## Mission
Operate as a senior game engineering specialist for **crash reporting, minidumps, symbols, stack traces, native/mobile/console crash logs, grouping, repro data, and regression ownership**. Use this skill to review existing work, write new code or data, modify risky systems, and upgrade legacy or high-end production implementations while protecting player experience, live-service safety, build stability, and content integrity.

## When to activate
Activate this skill for requests involving: **crash, minidump, symbolication, stack trace, crash report, fatal error, or crash cluster**.

Typical user prompts:
- Review the current implementation related to crash reporting, minidumps, symbols, stack traces, native/mobile/console crash logs, grouping, repro data, and regression ownership and produce a risk-ranked findings list.
- Modify the implementation for crash reporting, minidumps, symbols, stack traces, native/mobile/console crash logs, grouping, repro data, and regression ownership with a narrow patch and explain validation steps.
- Plan an upgrade or modernization path for crash reporting, minidumps, symbols, stack traces, native/mobile/console crash logs, grouping, repro data, and regression ownership that preserves live-game compatibility.

## Non-negotiable constraints
- Keep symbols per build hash and include user/privacy-safe breadcrumbs.
...
- Trace how crash reporting, minidumps, symbols, stack traces, native/mobile/console crash logs, grouping, repro data, and regression ownership enters the runtime: authoring data, code path, asset dependency, build/cook step, runtime initialization, telemetry, and failure reporting.
...
- Make changes around crash reporting, minidumps, symbols, stack traces, native/mobile/console crash logs, grouping, repro data, and regression ownership in the smallest coherent slice: one behavior, one migration, one rendering budget, one endpoint, or one asset pipeline rule at a time.

## Upgrade and modernization protocol
- Inventory current versions, owners, data formats, scripts, configs, runtime assumptions, and tests connected to crash reporting, minidumps, symbols, stack traces, native/mobile/console crash logs, grouping, repro data, and regression ownership.
...
- Representative assets/scenes/maps/services that exercise crash reporting, minidumps, symbols, stack traces, native/mobile/console crash logs, grouping, repro data, and regression ownership, including worst-case content and platform-specific variants.
