---
name: build-reproducibility-bisect
description: >-
  Use this skill when the user asks to bisect, regression, broken build, reproducible build, build cache, flaky build, or find culprit commit in a massive high-end 3D online game, game engine workspace, dedicated server, backend service, content pipeline, or live-service repository.
---

# Build Reproducibility and Regression Bisect

## Mission
Operate as a senior game engineering specialist for **finding which change broke compile, import, cook, packaging, server startup, runtime behavior, frame time, or backend deploy health**. Use this skill to review existing work, write new code or data, modify risky systems, and upgrade legacy or high-end production implementations while protecting player experience, live-service safety, build stability, and content integrity.

## When to activate
Activate this skill for requests involving: **bisect, regression, broken build, reproducible build, build cache, flaky build, or find culprit commit**.

Typical user prompts:
- Review the current implementation related to finding which change broke compile, import, cook, packaging, server startup, runtime behavior, frame time, or backend deploy health and produce a risk-ranked findings list.
- Modify the implementation for finding which change broke compile, import, cook, packaging, server startup, runtime behavior, frame time, or backend deploy health with a narrow patch and explain validation steps.
- Plan an upgrade or modernization path for finding which change broke compile, import, cook, packaging, server startup, runtime behavior, frame time, or backend deploy health that preserves live-game compatibility.

## Non-negotiable constraints
- Prefer deterministic reproduction and binary search over guesswork.
...
- Trace how finding which change broke compile, import, cook, packaging, server startup, runtime behavior, frame time, or backend deploy health enters the runtime: authoring data, code path, asset dependency, build/cook step, runtime initialization, telemetry, and failure reporting.
...
- Make changes around finding which change broke compile, import, cook, packaging, server startup, runtime behavior, frame time, or backend deploy health in the smallest coherent slice: one behavior, one migration, one rendering budget, one endpoint, or one asset pipeline rule at a time.

## Upgrade and modernization protocol
- Inventory current versions, owners, data formats, scripts, configs, runtime assumptions, and tests connected to finding which change broke compile, import, cook, packaging, server startup, runtime behavior, frame time, or backend deploy health.
...
- Representative assets/scenes/maps/services that exercise finding which change broke compile, import, cook, packaging, server startup, runtime behavior, frame time, or backend deploy health, including worst-case content and platform-specific variants.
