---
name: regionalization-latency-cost
description: >-
  Use this skill when the user asks to region, latency, data residency, edge, cross-region, cloud cost, bandwidth cost, or regional matchmaking in a massive high-end 3D online game, game engine workspace, dedicated server, backend service, content pipeline, or live-service repository.
---

# Regionalization, Latency, and Cost Optimization

## Mission
Operate as a senior game engineering specialist for **region selection, edge routing, data residency, matchmaking pools, cross-region failover, cloud cost, bandwidth cost, and player latency**. Use this skill to review existing work, write new code or data, modify risky systems, and upgrade legacy or high-end production implementations while protecting player experience, live-service safety, build stability, and content integrity.

## When to activate
Activate this skill for requests involving: **region, latency, data residency, edge, cross-region, cloud cost, bandwidth cost, or regional matchmaking**.

Typical user prompts:
- Review the current implementation related to region selection, edge routing, data residency, matchmaking pools, cross-region failover, cloud cost, bandwidth cost, and player latency and produce a risk-ranked findings list.
- Modify the implementation for region selection, edge routing, data residency, matchmaking pools, cross-region failover, cloud cost, bandwidth cost, and player latency with a narrow patch and explain validation steps.
- Plan an upgrade or modernization path for region selection, edge routing, data residency, matchmaking pools, cross-region failover, cloud cost, bandwidth cost, and player latency that preserves live-game compatibility.

## Non-negotiable constraints
- Balance latency, fairness, data laws, and operating cost.
...
- Trace how region selection, edge routing, data residency, matchmaking pools, cross-region failover, cloud cost, bandwidth cost, and player latency enters the runtime: authoring data, code path, asset dependency, build/cook step, runtime initialization, telemetry, and failure reporting.
...
- Make changes around region selection, edge routing, data residency, matchmaking pools, cross-region failover, cloud cost, bandwidth cost, and player latency in the smallest coherent slice: one behavior, one migration, one rendering budget, one endpoint, or one asset pipeline rule at a time.

## Upgrade and modernization protocol
- Inventory current versions, owners, data formats, scripts, configs, runtime assumptions, and tests connected to region selection, edge routing, data residency, matchmaking pools, cross-region failover, cloud cost, bandwidth cost, and player latency.
...
- Representative assets/scenes/maps/services that exercise region selection, edge routing, data residency, matchmaking pools, cross-region failover, cloud cost, bandwidth cost, and player latency, including worst-case content and platform-specific variants.
