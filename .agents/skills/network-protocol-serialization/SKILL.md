---
name: network-protocol-serialization
description: >-
  Use this skill when the user asks to network protocol, serialization, packet, RPC payload, binary format, protobuf, flatbuffers, compression, or protocol version in a massive high-end 3D online game, game engine workspace, dedicated server, backend service, content pipeline, or live-service repository.
---

# Network Protocol and Serialization

## Mission
Operate as a senior game engineering specialist for **binary protocols, schema versioning, packet layout, compression, encryption, endian/packing, RPC payloads, backward compatibility, and fuzzing**. Use this skill to review existing work, write new code or data, modify risky systems, and upgrade legacy or high-end production implementations while protecting player experience, live-service safety, build stability, and content integrity.

## When to activate
Activate this skill for requests involving: **network protocol, serialization, packet, RPC payload, binary format, protobuf, flatbuffers, compression, or protocol version**.

Typical user prompts:
- Review the current implementation related to binary protocols, schema versioning, packet layout, compression, encryption, endian/packing, RPC payloads, backward compatibility, and fuzzing and produce a risk-ranked findings list.
- Modify the implementation for binary protocols, schema versioning, packet layout, compression, encryption, endian/packing, RPC payloads, backward compatibility, and fuzzing with a narrow patch and explain validation steps.
- Plan an upgrade or modernization path for binary protocols, schema versioning, packet layout, compression, encryption, endian/packing, RPC payloads, backward compatibility, and fuzzing that preserves live-game compatibility.

## Non-negotiable constraints
- Version every protocol and validate untrusted payloads before use.
...
- Trace how binary protocols, schema versioning, packet layout, compression, encryption, endian/packing, RPC payloads, backward compatibility, and fuzzing enters the runtime: authoring data, code path, asset dependency, build/cook step, runtime initialization, telemetry, and failure reporting.
...
- Make changes around binary protocols, schema versioning, packet layout, compression, encryption, endian/packing, RPC payloads, backward compatibility, and fuzzing in the smallest coherent slice: one behavior, one migration, one rendering budget, one endpoint, or one asset pipeline rule at a time.

## Upgrade and modernization protocol
- Inventory current versions, owners, data formats, scripts, configs, runtime assumptions, and tests connected to binary protocols, schema versioning, packet layout, compression, encryption, endian/packing, RPC payloads, backward compatibility, and fuzzing.
...
- Representative assets/scenes/maps/services that exercise binary protocols, schema versioning, packet layout, compression, encryption, endian/packing, RPC payloads, backward compatibility, and fuzzing, including worst-case content and platform-specific variants.
